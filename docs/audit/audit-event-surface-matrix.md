...
| `KYB_APPROVED` | SEED_ONLY | — | — | — | — | — | — | — | — | No writer at all |
...
| `APPLICATION_REJECTED` | LIVE | Admin rejected the application | Admin | application_logs | Y | Y | n/a | `application_rejected` | Y | Reason not stored on the log row |
...
| `APPLICATION_APPROVED` | DEAD | — | — | — | Y *(label only)* | Y *(label only)* | n/a | `application_approved` *(dead)* | Y *(label only)* | Label on **all four** surfaces + in the issuer query allowlist; no writer, so it can never render |
...
| `CONTRACT_OFFER_REJECTED` | **DEAD** | — | — | — | Y *(label only)* | Y *(label only)* | n/a | — | Y *(label only)* | **Issuer decline writes `CONTRACT_WITHDRAWN` instead.** Label on all four surfaces + in the issuer query allowlist |
...
| `ISSUER_RESIDUAL_WITHDRAWAL_CREATED` | **DEAD** | — | — | — | — | — | — | — | — | Only in a sort-priority list |
...
| `USER_COMPLETED` | DEAD here | The only writer is `regtank/webhook-handler-dev.ts` (~492) and it targets **`onboarding_logs` on the dev database** (`DATABASE_URL_DEV`) | `FINAL_APPROVAL_COMPLETED` |
...
| `KYB_APPROVED` | SEED_ONLY | No writer anywhere | none — corporate approval flows through `ONBOARDING_APPROVED` |
...
| `APPLICATION_REJECTED` | LIVE | Admin sets status `REJECTED` | Admin | `admin/service.ts:updateApplicationStatus` (~6655) | **No rejection reason on the row** — open gap |
...
| `ISSUER_RESIDUAL_WITHDRAWAL_CREATED` | **DEAD** | No writer. Appears only in `admin-note-events-sorting.ts:38` | — | — | *(fallback)* | — | NO |
...
| `login_new_device` | **DEAD** | `Login from New Device` | `A new login was detected on {deviceName} from {location} at {time}.` | — | *(would be platform + email)* | none — no device-fingerprinting code exists |
| `kyc_approved` | **DEAD** | `Identity Verification Approved` | `Hello {userName}, your identity verification has been approved.` | — | — | none — superseded by `onboarding_approved` |
| `kyc_rejected` | **DEAD** | `Identity Verification Rejected` | `Hello {userName}, your identity verification was rejected.` + ` Reason: {reason}` | — | — | none — superseded by `onboarding_rejected` |
...
| `application_approved` | **DEAD** | `Application Approved` | `Your application {ref} has been approved.` | — | — | none — the `APPLICATION_APPROVED` event is dead too |
