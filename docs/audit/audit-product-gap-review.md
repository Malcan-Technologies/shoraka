...
| Dead / non-live events (declared, never written, or unreachable) | ~~17 confirmed~~ ~~19 non-live values~~ ~~20 non-live values~~ **24 non-live values, using precise classifications** — see matrix §7.1. Includes `AML_APPROVED` (reclassified **UNREACHABLE** 2026-08-24) and `access_logs.ROLE_ADDED`/`ROLE_REMOVED`/`ONBOARDING_RESET` + `onboarding_logs.ONBOARDING_RESET` (reclassified **UNREACHABLE** 2026-08-25) | LOW (no user impact; cleanup candidates only — all four are dormant plumbing with a real backend writer, not abandoned enum values) |
...
| Dead/seed/dev/unreachable event values | Full enumeration already lives in §5 above and in `audit-event-surface-matrix.md` §7.1 (the authoritative count, now 24 non-live values after the `AML_APPROVED` (2026-08-24) and `ROLE_ADDED`/`ROLE_REMOVED`/`ONBOARDING_RESET` (2026-08-25) reclassifications) | Cross-referenced, not duplicated here |
...
corrected above; **item 8 was investigated and classified as intentional-by-design**. That leaves
only **item 10 open**:
...
across the eight stores** (2026-08-24: `AML_APPROVED` reclassified LIVE → UNREACHABLE, +1;
2026-08-25: `access_logs.ROLE_ADDED`, `access_logs.ROLE_REMOVED`, `access_logs.ONBOARDING_RESET`,
`onboarding_logs.ONBOARDING_RESET` reclassified LIVE → UNREACHABLE, +4), using the precise
