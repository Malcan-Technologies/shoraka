...
| Dead / non-live events (declared, never written, or unreachable) | ~~17 confirmed~~ ~~19 non-live values~~ **20 non-live values, using precise classifications** — see matrix §7.1. Includes `AML_APPROVED`, newly reclassified **UNREACHABLE** on 2026-08-24 | LOW (no user impact; cleanup candidates only, except `AML_APPROVED` which is dormant manual-override plumbing) |
...
| Dead/seed/dev/unreachable event values | Full enumeration already lives in §5 above and in `audit-event-surface-matrix.md` §7.1 (the authoritative count, now 20 non-live values after the `AML_APPROVED` reclassification) | Cross-referenced, not duplicated here |
...
| 3 | ~~Admin Access-log filter dropdown lists `KYC_STATUS_UPDATED` (never written) but omits live `ROLE_ADDED`, `ROLE_REMOVED`, `PROFILE_UPDATED`, `ONBOARDING_RESET`~~ **RESOLVED (2026-08-24)** — `ACCESS_EVENT_TYPES` (moved into `use-access-logs.ts`, exported for testing) now includes all four live writers and excludes `KYC_STATUS_UPDATED` from the query filter (its dropdown option and historical-row label are kept, since seed rows may still exist). `EventType` in `packages/types/src/admin.ts` gained `ROLE_REMOVED`/`ONBOARDING_RESET`. Query/filter only; no writer or enum removal | `use-access-logs.ts:15-23` | ~~VISIBILITY_MISMATCH~~ **RESOLVED** |
...
stores**~~ **Total: 20 non-live values across the eight stores** (2026-08-24: `AML_APPROVED`
reclassified LIVE → UNREACHABLE, +1), using the precise classifications above. The full per-store
breakdown lives in [`audit-event-surface-matrix.md`](./audit-event-surface-matrix.md) §7.1, which
is now the authoritative count. Per the preservation contract, **none were removed** — dead enum
members, the unreachable `AML_APPROVED`/`PRODUCT_INACTIVATED`/`PRODUCT_REACTIVATED` writers, and
unused columns are cleanup/wiring candidates for a future, separate product decision, not something
to touch here.
