...
| `SETTLEMENT_TRUSTEE_LETTER_GENERATED` / `SETTLEMENT_TRUSTEE_EMAIL_SENT` / `SETTLEMENT_TRUSTEE_LETTER_SUBMITTED` / `SETTLEMENT_TRUSTEE_INSTRUCTION_COMPLETED` | Trustee letter + operational email lifecycle | Admin service methods | Admin | `s3Key`, settlement id, SES `messageId`, `resend` | ENOUGH. Live IDs are `SETTLEMENT_TRUSTEE_*`. Historical rows may still store `SERVICE_FEE_TRUSTEE_LETTER_GENERATED` / `_EMAIL_SENT` / `_LETTER_SUBMITTED` / `_INSTRUCTION_COMPLETED` |
