...
|---|---|---|---|---|---|
| `LOGIN` | User login (OAuth callback) | `cognito.routes.ts` OAuth callback | Subject | `portal` from OAuth state (null if unknown), IP/UA/device, `success`, `metadata.{requestedRole,roles,portal,stateId}`. **Does not store `activeRole`** — that was a copy of `requestedRole`, not a session persona. `POST /v1/auth/sync-user` does not write LOGIN. | ENOUGH |
| `LOGOUT` | User logout | `cognito.routes.ts`; `auth/service.ts:logout` | Subject | `portal` from `?portal=` or Origin/Referer (never `roles[0]`), IP/UA/device. Cognito logout metadata `{roles,portal}`. Service logout metadata `{activeRole?,portal}` where `activeRole` is `req.activeRole` or last session role (observed, not invented). | ENOUGH |
| `SIGNUP` | First CashSouk user establishment | `cognito.routes.ts` when the user row is created and no successful SIGNUP exists | Subject | Same as login success (`requestedRole`, `roles`, `portal`, `stateId` — no fake `activeRole`) | ENOUGH |
...
**Portal vs role (Access):** `portal` is which CashSouk frontend initiated the request (investor / issuer / admin, or null when unknown). It is never inferred from `user.roles[0]` and is never defaulted to Investor. `requestedRole` is the OAuth routing persona the user asked for. `roles` is the stored user role array. Those are separate facts. Local landing (`localhost:3000` / www) is not a portal.
...
| `EMAIL_VERIFIED` | Email verification success/fail | `auth/service.ts:verifyEmail` | Subject | `metadata.{email,reason,success?}` | ENOUGH (raw ID is `EMAIL_VERIFIED`, not `EMAIL_CHANGED`) |
...
| `INVESTMENT_COMMITTED` | Investor commits | `commitInvestment` | Investor | `investmentId`, `investorOrganizationId`, `amount`, `prospectusPublicationId`, `prospectusAcknowledgedAt` | ENOUGH |
...
| `SETTLEMENT_TRUSTEE_LETTER_GENERATED` / `SETTLEMENT_TRUSTEE_EMAIL_SENT` / `SETTLEMENT_TRUSTEE_LETTER_SUBMITTED` / `SETTLEMENT_TRUSTEE_INSTRUCTION_COMPLETED` | Trustee letter + operational email lifecycle | `generateSettlementTrusteeLetter` / `resendSettlementTrusteeEmail` / `markSettlementTrusteeLetterSubmitted` / `markSettlementTrusteeInstructionCompleted` | Admin | `s3Key`, settlement id, `settlementReference` (email), SES `messageId`, `resend` | ENOUGH. Canonical IDs are `SETTLEMENT_TRUSTEE_*`. Persistence/HTTP use `settlement_trustee_*` / `/settlement-trustee/`. |
