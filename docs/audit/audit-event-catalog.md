...
|---|---|---|---|---|---|
| `LOGIN` | User login (OAuth callback) | `cognito.routes.ts` OAuth callback | Subject | `portal` from OAuth state (null if unknown), IP/UA/device, `success`, `metadata.{requestedRole,activeRole,roles,portal}` | ENOUGH |
| `LOGOUT` | User logout | `cognito.routes.ts`; `auth/service.ts:logout` | Subject | `portal` from `?portal=` or Origin/Referer (never `roles[0]`), IP/UA/device, `metadata.{roles,portal}` | ENOUGH |
| `SIGNUP` | First CashSouk user establishment | `cognito.routes.ts` when the user row is created and no successful SIGNUP exists | Subject | Same as login success | ENOUGH |
