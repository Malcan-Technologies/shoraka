...
| `SERVICE_FEE_TRUSTEE_LETTER_GENERATED` / `SETTLEMENT_TRUSTEE_EMAIL_SENT` / `SERVICE_FEE_TRUSTEE_LETTER_SUBMITTED` / `SERVICE_FEE_TRUSTEE_INSTRUCTION_COMPLETED` | Trustee letter + operational email lifecycle | Admin service methods | Admin | `s3Key`, settlement id, SES `messageId`, `resend` | ENOUGH. Email live ID is `SETTLEMENT_TRUSTEE_EMAIL_SENT`; `SERVICE_FEE_TRUSTEE_EMAIL_SENT` remains on historical rows |
