...
The issuer **Review offer** modal catches that code, shows a QR step, and polls until verification completes. The user then continues to SigningCloud for the offer letter.
...
sequenceDiagram
  ...
  Desktop->>API: POST /v1/ekyc/session (auth, issuerOrganizationId)
  ...
  API->>SC: POST ekyc/submitResult (encryptedData, name, icNumber)
...
2. If eKYC is missing, modal step switches to **ekyc** (`ReviewOfferModal` + `useEkycFlow`).
3. `createEkycSession({ issuerOrganizationId, force? })` runs with the active org from `useOrganization()`.
4. QR encodes `{issuerOrigin}/ekyc/capture.html?token=…&endpoint=…&api={API_URL}`.
5. Desktop polls `GET /v1/ekyc/status?token=` every 2.5s until status is not `pending`.
6. On `verified`, user taps **Continue to signing** → `start-signing` APIs → browser redirect to SigningCloud.

Pending QR reuse: without `force: true`, an existing fresh pending session (updated within 25 minutes) returns the same `url` and `token` instead of calling SigningCloud again.
...
1. Loads WiseAI SDK (`WAILib.SDK`) with `docType: "mykad"`.
2. On capture success, posts SDK payload to `POST /v1/ekyc/complete`.
3. Shows outcome from API response:
   - `verified` → success overlay
   - `failed` → failed overlay (identity mismatch / vendor rejection)
...
MyKad **name** and **IC** sent to SigningCloud `submitResult` come from issuer organization data populated by RegTank onboarding — not from SDK OCR fields.
...
| Step | Function | Scope |
|------|----------|-------|
| Session create (`POST /session`) | `resolveIssuerEkycIdentityForOrganization` | Active org only — fail early if MyKad details missing; org id stored on `signingcloud_ekyc` |
| Complete (`POST /complete`) | `resolveIssuerEkycIdentityForOrganization` | Same org as stored on the session row |

`issuerOrganizationId` on session create is stored on `signingcloud_ekyc.issuer_organization_id` and used again on complete so name/IC match the org validated when the QR was generated.
...
| `POST` | `/session` | Required | Start or reuse session; body `{ issuerOrganizationId, force? }` |
| `GET` | `/status?token=` | None | Poll session status (desktop + debugging) |
| `POST` | `/complete` | None | Submit WiseAI capture result |
| `POST` | `/fail` | None | Record client-side capture failure |

Shared types: `packages/types` — `EkycSession`, `EkycSessionStatus`, `EkycMeStatus`.
...
2. Extract `encryptedData` from SDK result (preferred) or plain JSON fallback.
3. Call SigningCloud `submitResult` with `country: "MYS"`, `ekycDocType: "mykad"`.
4. If `userVerificationSuccess !== true` → DB `failed` (intentional: vendor may return HTTP 200 with `userVerificationSuccess: false`).
5. Otherwise → DB `verified` with `completed_at`.
...
| `failed` | Capture submitted but identity not verified (e.g. IC mismatch) |
| `error` | Setup/submit error or client-reported failure via `/fail` |

One row per `user_id`. Columns: `issuer_organization_id`, `session_token`, `sdk_endpoint`, `doc_type` (always `mykad`), `last_error`, `completed_at`, timestamps.
...
5. **submitResult** — `POST /signserver/v1/user/ekyc/submitResult` with encrypted SDK payload plus RegTank-resolved `name` and `icNumber`
...
| Tests | `service.test.ts`, `resolve-issuer-ekyc-identity.test.ts`, `signingcloud-ekyc.test.ts` |
...
4. Scan QR with a phone that can reach issuer origin and API URL.
...
| `failed` after capture | IC/name from RegTank did not match captured MyKad (SigningCloud `userVerificationSuccess: false`) |
