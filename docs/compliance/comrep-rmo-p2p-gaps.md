...
| 4 | **Investor classification is a boolean plus a separate SC enum.** `is_sophisticated_investor` remains the product flag. `sc_investor_category` is stored independently and is only auto-filled when the mapping is unique (HNW vs Accredited from distinct reasons; Non-sophisticated entity when a company is not product-sophisticated). Angel vs Retail, mixed HNW+Accredited, and sophisticated companies are still asked explicitly. | Completeness requires a valid SC enum for the org type / sophisticated flag. | Keep the two fields independent. Do not map `sophisticated = true` to one SC category. |
...
| Investor Types — Angel / Retail / Sophisticated, split by invested vs not-yet-invested | 18 | Partial | `sc_investor_category` is stored and required for profile completeness. It is not auto-derived from `is_sophisticated_investor`. Export still needs invested vs not-yet-invested splits. | Reporting query over the enum. |
...
| Type of Investor (6-value SC enum: Angel, Retail, Sophisticated ×3, Non-sophisticated entity) | 44–45 | Partial | `sc_investor_category` is stored. Unique cases are derived from onboarding reasons; Angel vs Retail and HNW Entity vs Non-Sophisticated Entity (when the company is product-sophisticated) still need an explicit choice. | Keep asking only the missing discriminator. Product `is_sophisticated_investor` stays unchanged. |
