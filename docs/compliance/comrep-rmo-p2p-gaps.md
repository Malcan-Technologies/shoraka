...
| Purpose of Fund Raising (Working Capital / Business Expansion / Others) | 32 | Stored | Dedicated `why_raising_funds.sc_purpose_of_fund_raising` (+ `sc_purpose_other` when Others). Free-text `financing_for` is preserved separately and frozen into `purpose_snapshot.financing_for`. | Application business-details step. Frozen onto `Note.purpose_snapshot`. |
