...
**Amendment exception:** when status is `AMENDMENT_REQUESTED` and the issuer keeps the stored `financial_year_end`, save uses `financialStatementsV2StoredSchema` (shape only) and the already-stored year keys. The issuer step snapshots those loaded rows at init and derives stored tabs from that snapshot only (not live `formsByYear`). Restore from the snapshot runs when the issuer re-enters the unchanged-FYE path; ordinary money-field edits do not retrigger restore. Changing FYE on amendment still requires the live window and live expected years. Drafts and initial SUBMIT stay strict.

Initial **SUBMIT** (not RESUBMIT) re-runs that schema and expected-year check when `financial_statements` is an active workflow step and a stored payload exists. The guard runs once, before paymaster linking, so a rejected submit does not persist contract/paymaster writes and a midnight boundary cannot fail a second evaluation after those writes. Failure is `400 VALIDATION_ERROR` with `Financial Statements: <message>` so a pre-window draft (e.g. FYE 31 Dec 2027) cannot be submitted by skipping the step.
