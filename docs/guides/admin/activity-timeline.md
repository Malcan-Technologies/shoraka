...
| `PAYMASTER_IDENTITY_UPDATED` | paymaster/service `updatePaymasterIdentity` / `verifyPaymaster` | ADMIN | Official legal name, country, or entity type changed. SSM is locked. A Verified edit also overlays official identity onto eligible working applications. Display: Paymaster Identity Updated. Admin: Paymaster → Activity; Application → Activity Timeline only when a real application context exists |
| `PAYMASTER_VERIFIED` | paymaster/service `verifyPaymaster` | ADMIN | First Unverified → Verified via Verify Paymaster. Display: Paymaster Identity Verified. Does not approve the application. Eligible working applications then use the official identity automatically. Admin: Application → Activity Timeline when application context exists; Paymaster → Activity |
| `PAYMASTER_IDENTITY_SYNCED` | paymaster/service `syncOfficialIdentityToEligibleApplications` | ADMIN | Auto-sync changed this unfinished application's working `customer_details` from the official Paymaster. Written only when that application's stored identity actually changed. Display: Paymaster Identity Synced. Reason/source: Official Paymaster identity updated. Admin: Application → Activity Timeline; Paymaster → Activity. Not customer visible. No notification |
| `PAYMASTER_IDENTITY_RESOLVED` | historical reader (no live writer) | ADMIN | Historical Admin overlay of official Paymaster identity onto an application's customer_details (former Use Verified Paymaster Details). Display: Paymaster Identity Resolved. Existing rows still render on Application → Activity Timeline and Paymaster → Activity. Not customer visible |
