...
| `PAYMASTER_IDENTITY_UPDATED` | paymaster/service `updatePaymasterIdentity` / `verifyPaymaster` | ADMIN | Official legal name, country, or entity type changed. SSM is locked. Display: Paymaster Identity Updated. Admin: Paymaster → Activity; Application → Activity Timeline only when a real application context exists |
| `PAYMASTER_VERIFIED` | paymaster/service `verifyPaymaster` | ADMIN | First Unverified → Verified via Verify Paymaster. Display: Paymaster Identity Verified. Does not approve the application. Admin: Application → Activity Timeline when application context exists; Paymaster → Activity |
| `PAYMASTER_IDENTITY_RESOLVED` | paymaster/service `applyVerifiedPaymasterIdentityToApplication` | ADMIN | Admin used official Paymaster identity on this application's submitted customer_details while the review section was still open. Display: Paymaster Identity Resolved. Does not notify the issuer. Admin: Application → Activity Timeline; Paymaster → Activity |
