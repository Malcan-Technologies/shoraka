...
Organization invitations (ISSUER and INVESTOR, shared `OrganizationService`): a new invite writes `ORGANIZATION_MEMBER_INVITED`. Same email + same role + valid pending invitation reuses the same invitation id/token/expiry/link and, after a successful email send, writes `ORGANIZATION_INVITATION_RESENT` (not a second INVITED). Same email + different role creates a new invitation and writes `ORGANIZATION_MEMBER_INVITED`. Revoked, expired, or accepted invitations are not reused. Resend does not rotate the token or extend expiry. A general/unbound invite (no member email; placeholder email; `memberEmail` null) still writes `ORGANIZATION_MEMBER_INVITED`; accept writes `ORGANIZATION_MEMBER_JOINED` with the actual member. Organization Copy Link writes **no** Security event (unlike Admin Copy Link, which writes `ADMIN_INVITATION_LINK_GENERATED`). Member remove → `ORGANIZATION_MEMBER_REMOVED`. Member leave → `ORGANIZATION_MEMBER_LEFT`. Role change → `ORGANIZATION_MEMBER_ROLE_UPDATED`. Ownership transfer → `ORGANIZATION_OWNERSHIP_TRANSFERRED`.
...
**Reserved/catalogued IDs:** 18 = original A039–A055 plus later-appended A175 (ONB-018 `ORGANIZATION_PROFILE_UPDATED_BY_ADMIN`). **Current active onboarding event types:** 15. **Retired / no current writer:** A040 `ONBOARDING_RESUMED`, A052 `CTOS_REPORT_RECEIVED`, A053 `CORPORATE_ENTITIES_UPDATED`. Historical rows remain readable. IDs are not reused. Overall audit catalogue is **178 reserved/catalogued IDs** (A001–A178), with **170 currently active event types** that have at least one writer (retired: A004 `USER_ROLE_ADDED`, A005 `ACTIVE_ROLE_CHANGED`, A010 `USER_EMAIL_VERIFIED`, A011 `EMAIL_VERIFICATION_FAILED`, A016 `USER_ROLES_UPDATED`, A040, A052, A053). Investor/Issuer signup email verification remains on Landing `/verify-email` via `POST /v1/auth/confirm-signup` / `POST /v1/auth/resend-signup-code`. That path does not write A010/A011. `POST /v1/auth/verify-email` has been removed.
