...
Reference for every combination of contract and invoice statuses and the resulting application status.

**Rule:** When contract exists, contract controls. Invoices do NOT override. When no contract, invoices control.
...
**Rule:** All same final → use that. Mix of APPROVED / REJECTED / WITHDRAWN → COMPLETED. Any non-final → keep current status.
...
| DRAFT | unchanged |
| AMENDMENT_REQUESTED | unchanged |

**Rule:** Contract in final state → use that. Contract active → keep current status.
...
**Rule:** Contract APPROVED + all invoices final (APPROVED/REJECTED/WITHDRAWN) → COMPLETED. If any invoice not final → keep current status.

---

## 5. Contract + Invoices — Contract Active (SUBMITTED, OFFER_SENT, etc.)

Contract not in final state. Invoices do NOT control. Application keeps current status.
...
**Rule:** When contract exists and is active, invoices cannot change application status. Return current status.

---

## 6. Cancel Application (Withdraw at Application Level)

**What it does:** Withdraws all active invoices and contract, then recalculates using the tables above.

| Before Cancel | Invoices After | Contract After | Application Result |
|---------------|----------------|----------------|--------------------|
| Contract SUBMITTED, Invoices SUBMITTED | WITHDRAWN | WITHDRAWN | WITHDRAWN |
| Contract APPROVED, Invoices SUBMITTED | WITHDRAWN | APPROVED (unchanged) | COMPLETED |
...
| Contract exists, all invoices REJECTED, contract APPROVED | COMPLETED |
| Contract exists, all invoices WITHDRAWN, contract SUBMITTED | unchanged (invoices do not override) |
| Contract WITHDRAWN, invoices APPROVED | WITHDRAWN (contract wins) |
...
**"Unchanged"** = returns `currentApplicationStatus` (the value passed into `computeApplicationStatus`).
