...
| Event ID | Event / Activity | System Event Code | Description | Trigger / Condition | Actor | Affected Record | Recorded Data | Record Source | Admin Location | Customer Visible | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| LOG-APP-001 | Application Created | `APPLICATION_CREATED` | Draft application created | Issuer creates draft (same transaction as the application row). Hourly job backfills historical missing rows only | Customer / System | Application | Application refs | `application_logs` | Application record - Activity Timeline | Yes | Customer title Application Started. Live create uses the issuer actor. Legacy repair uses source Internal process and a null actor |
