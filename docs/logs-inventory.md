...
| LOG-FAC-009 | Paymaster Created | `PAYMASTER_CREATED` | New Paymaster master created Unverified from issuer Customer Details | Issuer submit/resubmit when SSM is not already on a master | Issuer | Paymaster + originating application | Paymaster id, SSM, legal name, Unverified | `application_logs` | Application record - Activity Timeline; Paymaster record - Activity | No | Created on submit, not draft save. Initial issuer link is included. Do not also write Linked to Issuer |
| LOG-FAC-010 | Paymaster Linked to Issuer | `PAYMASTER_LINKED_TO_ISSUER` | New issuer link created on an existing Paymaster master | Issuer submit/resubmit when this issuer did not already have a link | Issuer | Paymaster + originating application | Link id, Paymaster id, related-party, verification status | `application_logs` | Application record - Activity Timeline; Paymaster record - Activity | No | Not written for same-issuer reuse or last-used updates |
| LOG-FAC-011 | Paymaster Identity Verified | `PAYMASTER_VERIFIED` | Admin reviewed Paymaster identity Unverified → Verified | Admin Verify Paymaster, optionally with confirmed legal name / country / entity type | Admin | Paymaster + originating application when supplied | Previous/new status, verified by | `application_logs` | Application record - Activity Timeline when application context exists; Paymaster record - Activity | No | Not application approval. Not Notice acknowledgement. Idempotent re-verify writes nothing. Identity field changes in the same action also write Paymaster Identity Updated. After verification, eligible unfinished applications whose working `customer_details` actually change also receive Paymaster Identity Synced. Completed/frozen applications and revision snapshots stay unchanged |
| LOG-FAC-012 | Paymaster Identity Synced | `PAYMASTER_IDENTITY_SYNCED` | This unfinished application's current Paymaster working details were automatically updated from the official Paymaster master | Auto-sync after Verify Paymaster or a later Verified-master edit, only when that application's `customer_details` identity fields changed | Admin (actor who changed the master) | Application whose working identity changed | Previous/new working identity fields, paymaster id, contract id, trigger, source `paymaster_auto_sync` | `application_logs` | Application record - Activity Timeline; Paymaster record - Activity | No | Audit only. No inbox, email, or SES. Not written for frozen/completed applications, unchanged working identity, or Unverified master edits. Does not replace Paymaster Identity Updated or Paymaster Identity Verified. Does not rewrite revision history |
| LOG-FAC-013 | Paymaster Identity Updated | `PAYMASTER_IDENTITY_UPDATED` | Admin changed official Paymaster legal name, country, or entity type | Admin Edit Paymaster Details, or Verify Paymaster when those fields change | Admin | Paymaster + originating application when the edit came from Application Review | Previous/new legal name, country, entity type, verification status | `application_logs` | Paymaster record - Activity; Application record - Activity Timeline only when a real application context exists | No | SSM cannot be changed. Verified masters stay Verified. A Verified edit also overlays official identity onto eligible working applications; those applications get Paymaster Identity Synced when their working data actually changes. Does not rewrite historical snapshots or notify customers |
...
| `PAYMASTER_IDENTITY_UPDATED` | Humanized token | Paymaster Identity Updated |
| `PAYMASTER_VERIFIED` | Paymaster Verified | Paymaster Identity Verified |
| `PAYMASTER_IDENTITY_SYNCED` | Humanized token | Paymaster Identity Synced |
| `PAYMASTER_IDENTITY_RESOLVED` | Humanized token | Paymaster Identity Resolved (historical) |
