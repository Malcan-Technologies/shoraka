...
export function useOrganizationMembers(organizationId: string | undefined) {
  ...
  const removeMemberMutation = useMutation({
    mutationFn: async (userId: string) => {
      ...
      const result = await apiClient.delete(
        `/v1/organizations/issuer/${organizationId}/members/${userId}`
      );
      if (!result.success) {
        throw new Error(result.error.message);
      }
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["issuer-organizations"] });
  ...
  const changeRoleMutation = useMutation({
    mutationFn: async ({ userId, role }: { userId: string; role: "ORGANIZATION_ADMIN" | "ORGANIZATION_MEMBER" }) => {
      ...
      const result = await apiClient.patch(
        `/v1/organizations/issuer/${organizationId}/members/${userId}/role`,
        { role }
      );
      if (!result.success) {
        throw new Error(result.error.message);
  ...
  const leaveMutation = useMutation({
    mutationFn: async () => {
      if (!organizationId) throw new Error("No organization selected");
      const result = await apiClient.post(`/v1/organizations/issuer/${organizationId}/leave`);
      if (!result.success) {
        throw new Error(result.error.message);
      }
      return result.data;
  ...
  const transferOwnershipMutation = useMutation({
    mutationFn: async (newOwnerId: string) => {
      ...
      const result = await apiClient.post(
        `/v1/organizations/issuer/${organizationId}/transfer-ownership`,
        { newOwnerId }
      );
      if (!result.success) {
        throw new Error(result.error.message);
