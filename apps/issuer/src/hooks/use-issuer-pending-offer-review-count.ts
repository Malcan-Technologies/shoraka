...
/** Counts applications that still need issuer action (offers and/or amendments). */
export function useIssuerPendingOfferReviewCount(organizationId: string | undefined): number {
  ...
  return useMemo(() => {
    ...
    return countIssuerApplicationsNeedingAction(normalizeApplicationsList(apiApplications));
