...
function SettlementSummaryBlock({ note }: { note: NoteListItem }) {
  ...
  const lines = issuerSettlementAllocationLines(note.settlementSummary);
  const profitAccrual = issuerSettlementProfitAccrualPeriod(note.settlementSummary);

  return (
    <div className="rounded-xl border p-3">
      <p className="text-meta text-muted-foreground">{ISSUER_SETTLEMENT_PAYOUT_INTRO}</p>
      <div className="mt-2 grid grid-cols-2 gap-2 text-meta sm:grid-cols-3">
        {lines.map((line) => (
          <div key={line.key}>
            <div className="text-muted-foreground">{line.label}</div>
            <div className="font-semibold tabular-nums">{formatCurrency(line.value)}</div>
          </div>
        ))}
      </div>
      {profitAccrual ? (
        <p className="mt-2 text-meta text-muted-foreground">
          <span className="font-medium text-foreground">Profit accrual period:</span> {profitAccrual}
        </p>
      ) : null}
    </div>
  );
