...
describe("prospectus review completion readiness", () => {
  ...
  it("does not treat incomplete optional sections as submit blockers", () => {
    ...
    expect(checklist.find((i) => i.id === "page3Paymaster")).toBeUndefined();
  ...
  it("routes remaining Page 3 missing sections to their existing tabs", () => {
  ...
  it("does not count Paymaster Grading or Confidence Grading as missing required fields", () => {
    const draft = completeOfficerDraft();
    draft.page2.invoicePaymaster = {
      deedOfAssignment: "Yes",
    };
    const missing = buildProspectusMissingRequiredFields(draft);
    expect(missing.some((m) => m.field === "Paymaster Grading")).toBe(false);
    expect(missing.some((m) => m.field === "Confidence Grading")).toBe(false);
    expect(missing.some((m) => m.section === "Page 3 Paymaster Grading")).toBe(false);
  ...
  it("does not include Paymaster Grading in the Page 3 overall missing total", () => {
    const draft = completeOfficerDraft();
