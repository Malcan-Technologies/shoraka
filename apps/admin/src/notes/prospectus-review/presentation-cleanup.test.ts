...
describe("prospectus review presentation cleanup", () => {
  ...
  it("Page 3 overview and coverage hide source lines; reused cells stay read-only", () => {
    ...
    expect(pageThree).not.toContain('id: "paymaster_grading"');
    expect(pageThree).not.toContain("Page 3 Paymaster Grading");
    expect(pageThree).not.toContain('label="Paymaster Grading"');
    expect(pageThree).not.toContain('label="Confidence Grading"');
    expect(pageThree).not.toContain("data-prospectus-page-three-paymaster-grading");
    expect(pageThree).not.toContain("page2.invoicePaymaster");
  ...
  it("does not require Paymaster Grading or Confidence Grading for completion", () => {
    expect(completion).not.toContain('id: "page3Paymaster"');
    expect(completion).not.toContain('tabId: "paymaster_grading"');
    expect(completion).not.toContain("Page 3 Paymaster Grading");
    expect(completion).not.toContain("Paymaster Grading");
    expect(completion).not.toContain("Confidence Grading");
