...
export function ApplicationFinancialReviewComparison({
  ...
  const mockFinancialPayload = USE_MOCK_FINANCIAL_RESUBMIT_COMPARISON
    ? buildMockFinancialResubmitPayload(MOCK_UNAUDITED_YEAR_COUNT)
    : null;

  const effectiveBeforeApp = !mockFinancialPayload
    ? beforeApp
    : { ...beforeApp, financial_statements: mockFinancialPayload!.before };

  const effectiveAfterApp = !mockFinancialPayload
    ? afterApp
    : { ...afterApp, financial_statements: mockFinancialPayload!.after };

  const effectiveIsPathChanged = (path: string) => {
  ...
  const beforeExtracted = extractQuestionnaireAndUnaudited(effectiveBeforeApp.financial_statements);
  const afterExtracted = extractQuestionnaireAndUnaudited(effectiveAfterApp.financial_statements);
  const beforeByYear = beforeExtracted.unauditedByYear;
  const afterByYear = afterExtracted.unauditedByYear;

  const beforeUnauditedKeys = sortedUnauditedYearKeys(beforeByYear);
  const afterUnauditedKeys = sortedUnauditedYearKeys(afterByYear);
  const unauditedSlots = buildUnauditedSlots(beforeUnauditedKeys, afterUnauditedKeys);
                ...
                <TableBody>
                  {(() => {
                    ...
                    const renderLabel = (key: string) => {
                      ...
                      if (!isEquityIfApplicableKey(key)) return base;
