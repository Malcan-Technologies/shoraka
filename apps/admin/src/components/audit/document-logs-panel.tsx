...
export function DocumentLogsPanel() {
  ...
  const handleExport = async (format: "csv" | "json") => {
    try {
      const blob = await getExportLogs({
        ...
        eventType:
          eventTypeFilter !== "all" ? (eventTypeFilter as DocumentEventType) : undefined,
        dateRange: dateRangeFilter as "24h" | "7d" | "30d" | "all",
        format,
      });
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm mt-1">Document changes will be recorded here</p>
                </TableCell>
              </TableRow>
            ) : (
              logs.map((log) => {
