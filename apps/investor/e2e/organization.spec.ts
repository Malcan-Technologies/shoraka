...
test.describe("Sidebar Onboarding State", () => {
  test.beforeEach(async ({ page }) => {
    await login(page);
  });
  ...
  test("should grey out sidebar items for non-onboarded organization", async ({ page }) => {
    ...
    // This will vary based on onboarding state
...
test.describe("Database Verification", () => {
  test("should create organization record in database", async ({ page, request }) => {
    ...
    // Note: This requires API access with the same auth token
    const cookies = await page.context().cookies();
    const sessionCookie = cookies.find(c => c.name.includes("session"));
    
    if (sessionCookie) {
      const response = await request.get("http://localhost:4000/v1/organizations/investor", {
        headers: {
