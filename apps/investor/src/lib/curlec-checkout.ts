...
function loadCurlecCheckoutScript(): Promise<void> {
  ...
  if (!scriptPromise) {
    scriptPromise = new Promise<void>((resolve, reject) => {
      ...
      script.onerror = () => reject(new Error("Failed to load Curlec checkout"));
 ...
 * @see https://curlec.com/docs/payments/payment-gateway/web-integration/standard/integration-steps/
 */
...
function resolveInvestorPortalOrigin(): string {
  const configured = process.env.NEXT_PUBLIC_INVESTOR_URL?.trim();
  if (configured) {
  ...
  // Preview URLs without explicit env config fall back to the browser origin.
  return currentOrigin;
}

export function buildDepositCallbackUrl(depositId: string, returnTo?: string): string {
  const params = new URLSearchParams({ depositId });
  if (returnTo) {
    params.set("returnTo", returnTo);
  }
  return `${resolveInvestorPortalOrigin()}/deposits/callback?${params.toString()}`;
}
