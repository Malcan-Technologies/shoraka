...
// AWS Cognito rate limit: Conservative 15-second cooldown
...
// Step 2: Enter verification code schema
...
function VerifyEmailPageContent() {
    ...
    try {
      ...
      if (!response.ok) {
        if (data.error?.code === "USER_NOT_FOUND") {
          ...
          setCooldownRemaining(60); // Force 1-minute cooldown
        } else if (data.error?.code === "ALREADY_CONFIRMED") {
          toast.error("Already verified", {
            description: "This account is already verified. Try logging in.",
          });
        } else {
  ...
  const onCodeSubmit = async (data: CodeFormValues) => {
    setIsSubmitting(true);
    try {
      const response = await fetch(`${API_URL}/v1/auth/confirm-signup`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        if (result.error?.code === "INVALID_CODE") {
          ...
          toast.error("Code expired", {
            description: "This code has expired. Click 'Resend' to get a new one.",
          });
      ...
      setTimeout(() => {
        const loginUrl = `${API_URL}/api/auth/login?role=${redirect.toUpperCase()}`;
