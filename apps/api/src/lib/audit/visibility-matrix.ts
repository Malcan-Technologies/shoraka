...
export const EVENT_CATALOGUE: Record<string, EventCatalogueEntry> = {
  ...
  PAYMASTER_IDENTITY_RESOLVED: entry(EVENT_LAYER.ADMIN_ACTIVITY, "application_logs", {
    lifecycle: EVENT_LIFECYCLE.HISTORICAL_READER,
    notes:
      "No live writer. Historical Admin Use Verified overlay. Kept so existing rows still render.",
  }),
  ...
  SIGNING_DOCUMENT_SIGNED: entry(EVENT_LAYER.APPLICATION_TIMELINE, "application_logs", USER),
  ...
  NOTE_OVERDUE: entry(EVENT_LAYER.NOTE_TIMELINE, "note_events", USER),
  NOTE_LATE: entry(EVENT_LAYER.NOTE_TIMELINE, "note_events", USER),
  NOTE_ARREARS: entry(EVENT_LAYER.NOTE_TIMELINE, "note_events", USER),
  NOTE_LETTER_SENT: entry(EVENT_LAYER.NOTE_TIMELINE, "note_events", USER),
  LATE_CHARGE_WAIVED: entry(EVENT_LAYER.NOTE_TIMELINE, "note_events", USER),
