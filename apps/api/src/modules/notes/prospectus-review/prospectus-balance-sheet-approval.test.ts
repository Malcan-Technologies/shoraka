...
describe("prospectus Balance Sheet approval validation", () => {
  ...
  it("requires only Quick Ratio for each year", () => {
    const draft = buildCompleteProspectusReviewDraft();
    ...
    expect(
      errors.some((e) => e.path === "page3.manualFinancialInputs.years.2022.quickRatio")
    ).toBe(true);
    expect(
      errors.some((e) => e.path === "page3.manualFinancialInputs.years.2024.quickRatio")
    ).toBe(true);
    expect(
      errors.some((e) => e.path === "page3.manualFinancialInputs.years.2023.quickRatio")
    ).toBe(false);
  });
