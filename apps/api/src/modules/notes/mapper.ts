...
function resolveSettlementSummary(note: NoteWithRelations) {
  ...
  return {
    ...
    excessLateChargeWaivedAmount: moneyToNumber(settlement.excess_late_charge_waived_amount),
...
export function mapNoteListItem(note: NoteWithRelations) {
  ...
  return {
    ...
    daysPastDue: note.days_past_due ?? 0,
    ...
    excessLateCharges: mapExcessLateChargesDto({
      ...
      excessLateChargeWaivedAmount: settlementSummary?.excessLateChargeWaivedAmount,
      noteReference: note.note_reference,
    }),
...
export async function mapNoteDetail(
  ...
  return applyNoteSourceDisplayReferences({
    ...
    overdueStartedAt: iso(note.overdue_started_at),
    lateStartedAt: iso(note.late_started_at),
    arrearsStartedAt: iso(note.arrears_started_at),
    daysPastDue: note.days_past_due ?? 0,
    indicativeTawidhAmount: moneyToNumber(note.indicative_tawidh_amount),
    indicativeGharamahAmount: moneyToNumber(note.indicative_gharamah_amount),
    indicativeAsOf: iso(note.indicative_as_of),
    servicingLetters: (note.servicing_letters ?? []).map((letter) => ({
      id: letter.id,
      noteId: letter.note_id,
      type: letter.type,
      generatedAt: letter.generated_at.toISOString(),
      sentAt: iso(letter.sent_at),
      triggeredBy: letter.triggered_by,
    })),
    lateChargeWaivers: (note.late_charge_waivers ?? []).map((waiver) => ({
      id: waiver.id,
      noteId: waiver.note_id,
      settlementId: waiver.settlement_id,
      tawidhWaivedAmount: moneyToNumber(waiver.tawidh_waived_amount),
      gharamahWaivedAmount: moneyToNumber(waiver.gharamah_waived_amount),
      reason: waiver.reason,
      waivedByAdminUserId: waiver.waived_by_admin_user_id,
    ...
    settlements: note.settlements.map((settlement) => ({
      ...
      excessLateChargePaidAmount: moneyToNumber(settlement.excess_late_charge_paid_amount),
      excessLateChargeWaivedAmount: moneyToNumber(settlement.excess_late_charge_waived_amount),
      daysPastDueAtPayment: settlement.days_past_due_at_payment,
      dpdBucketAtPayment: settlement.dpd_bucket_at_payment,
      excessTawidhAmount: moneyToNumber(settlement.excess_tawidh_amount),
      excessGharamahAmount: moneyToNumber(settlement.excess_gharamah_amount),
      actualSettlementDate: iso(settlement.actual_settlement_date),
