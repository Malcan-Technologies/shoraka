...
describe("RegTank master identity persistence stamps profile_field_sources", () => {
  ...
  it("does not overwrite user gender value nor provenance when already filled (investor)", async () => {
    mockInvestorFindUnique.mockResolvedValue({
      id: "org-1",
      type: OrganizationType.PERSONAL,
      name: "Personal Org",
    ...
    await (service as any).extractAndUpdateOrganizationData(
      "org-1",
      "investor",
      makeRegTankDetails({ userProfile: { gender: "MALE" } }),
      undefined
    );

    const updateData = mockPersistOrganizationUpdateAndOnboardingLogs.mock.calls[0]?.[0]?.data;
    expect(updateData.gender).toEqual("FEMALE");
    expect((updateData.profile_field_sources as any)?.gender?.source).toBe("USER");
  });

  it("does not overwrite user nationality value nor provenance when already filled (investor)", async () => {
    mockInvestorFindUnique.mockResolvedValue({
      id: "org-1",
      type: OrganizationType.PERSONAL,
      name: "Personal Org",
    ...
    await (service as any).extractAndUpdateOrganizationData(
      "org-1",
      "investor",
      makeRegTankDetails({ userProfile: { nationality: "SINGAPORE" } }),
      undefined
    );

    const updateData = mockPersistOrganizationUpdateAndOnboardingLogs.mock.calls[0]?.[0]?.data;
    expect(updateData.nationality).toEqual("MALAYSIA");
    expect((updateData.profile_field_sources as any)?.nationality?.source).toBe("USER");
  });

  it("does not overwrite user identityNumber value or provenance when already filled (investor)", async () => {
    mockInvestorFindUnique.mockResolvedValue({
      id: "org-1",
      type: OrganizationType.PERSONAL,
      name: "Personal Org",
    ...
    await (service as any).extractAndUpdateOrganizationData(
      "org-1",
      "investor",
      makeRegTankDetails({ userProfile: { documentNum: "900202022222" } }),
      undefined
    );

    const updateData = mockPersistOrganizationUpdateAndOnboardingLogs.mock.calls[0]?.[0]?.data;
    expect(updateData.document_number).toEqual("800101011234");
    expect((updateData.profile_field_sources as any)?.identityNumber?.source).toBe("USER");
  });
