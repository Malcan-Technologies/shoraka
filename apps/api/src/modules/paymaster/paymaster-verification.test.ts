import {
  PAYMASTER_NOT_VERIFIED_CODE,
  PAYMASTER_NOT_LINKED_CODE,
  RELATED_PARTY_REQUIRED_CODE,
  submittedIdentityDiffersFromVerified,
} from "@cashsouk/types";

jest.mock("../../lib/prisma", () => ({
  prisma: {
    paymaster: {
      findUnique: jest.fn(),
      findFirst: jest.fn(),
      create: jest.fn(),
      update: jest.fn(),
    },
    issuerPaymasterLink: {
      findUnique: jest.fn(),
      findMany: jest.fn(),
      create: jest.fn(),
      update: jest.fn(),
      deleteMany: jest.fn(),
    },
    contract: {
      findFirst: jest.fn(),
      findUnique: jest.fn(),
      findMany: jest.fn(),
      update: jest.fn(),
      count: jest.fn(),
    },
    user: {
      findUnique: jest.fn(),
    },
    note: {
      findMany: jest.fn(),
      update: jest.fn(),
      count: jest.fn(),
    },
    application: {
      update: jest.fn(),
      findUnique: jest.fn(),
    },
    applicationRevision: {
      update: jest.fn(),
    },
    $transaction: jest.fn(),
  },
}));

jest.mock("./identity-audit", () => {
  const actual = jest.requireActual("./identity-audit");
  return {
    ...actual,
    writePaymasterIdentityApplicationLog: jest.fn(async () => undefined),
  };
});

import { prisma } from "../../lib/prisma";
import { AppError } from "../../lib/http/error-handler";
import { ActivityPortal, ApplicationLogEventType } from "../applications/logs/types";
import { writePaymasterIdentityApplicationLog } from "./identity-audit";
import {
  assertApplicationPaymasterReadyForOffer,
  isPaymasterWorkingIdentityEligible,
  linkPaymasterForApplicationSubmission,
  listIssuerPaymasters,
  lookupIssuerPaymasterByRegistration,
  lookupPaymasterByRegistration,
  persistDraftCustomerDetails,
  resolvePaymasterFromCustomerDetails,
  resolvePersistedDraftIdentity,
  shouldLockPaymasterSwitching,
  shouldRetainLinkedFacilityPaymaster,
  updatePaymasterIdentity,
  verifyPaymaster,
} from "./service";

const issuerOrganizationId = "org-a";
const actorUserId = "A1B2C";
const applicationId = "app-1";
const writeLogMock = writePaymasterIdentityApplicationLog as jest.MockedFunction<
  typeof writePaymasterIdentityApplicationLog
>;

function paymasterRow(overrides: Record<string, unknown> = {}) {
  return {
    id: "pm_verified",
    legal_name: "ABC Trading Sdn Bhd",
    registration_number: "202134567890",
    registration_country: "MY",
    entity_type: "Private Limited Company (Sdn Bhd)",
    verification_status: "VERIFIED",
    verified_at: new Date("2026-06-01T00:00:00.000Z"),
    verified_by_user_id: actorUserId,
    source: "ISSUER_APPLICATION",
    created_at: new Date("2026-01-01T00:00:00.000Z"),
    updated_at: new Date("2026-01-02T00:00:00.000Z"),
    ...overrides,
  };
}

function detailIncludes(row: ReturnType<typeof paymasterRow>) {
  return {
    ...row,
    issuer_links: [],
    assignment_notices: [],
    contracts: [],
    notes: [],
  };
}

function matchingDetails(row: ReturnType<typeof paymasterRow>, extra: Record<string, unknown> = {}) {
  return {
    name: row.legal_name,
    entity_type: row.entity_type,
    ssm_number: row.registration_number,
    country: row.registration_country,
    is_related_party: false,
    ...extra,
  };
}

function workingApp(
  status = "UNDER_REVIEW",
  financing_structure: unknown = { structure_type: "new_contract" },
  id = applicationId
) {
  return { id, status, financing_structure };
}

function linkedContract(params: {
  id?: string;
  status?: string;
  details: Record<string, unknown>;
  applications?: ReturnType<typeof workingApp>[];
}) {
  return {
    id: params.id ?? "ctr-1",
    status: params.status ?? "DRAFT",
    customer_details: params.details,
    applications: params.applications ?? [workingApp()],
  };
}

function syncedApplicationIds() {
  return writeLogMock.mock.calls
    .filter((call) => call[0].eventType === ApplicationLogEventType.PAYMASTER_IDENTITY_SYNCED)
    .map((call) => call[0].applicationId);
}

function applicationWithCustomerDetails(
  details: Record<string, unknown>,
  paymasterId: string,
  id = applicationId
) {
  return {
    id,
    contract: {
      paymaster_id: paymasterId,
      customer_details: details,
    },
  };
}

function mockPaymasterReads(state: { row: ReturnType<typeof paymasterRow> }) {
  (prisma.paymaster.findUnique as jest.Mock).mockImplementation(
    async (args: { include?: unknown; select?: { verification_status?: boolean } }) => {
      if (args?.select?.verification_status) {
        return { verification_status: state.row.verification_status };
      }
      if (args?.include) return detailIncludes(state.row);
      return state.row;
    }
  );
  (prisma.paymaster.update as jest.Mock).mockImplementation(
    async ({ data }: { data: Record<string, unknown> }) => {
      state.row = { ...state.row, ...data };
      return state.row;
    }
  );
}

describe("Paymaster verification and SSM-first reuse", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (prisma.$transaction as jest.Mock).mockImplementation(async (fn: (tx: typeof prisma) => unknown) =>
      fn(prisma)
    );
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue(null);
    (prisma.issuerPaymasterLink.create as jest.Mock).mockResolvedValue({ id: "link-new" });
    (prisma.issuerPaymasterLink.update as jest.Mock).mockResolvedValue({ id: "link-existing" });
    (prisma.issuerPaymasterLink.deleteMany as jest.Mock).mockResolvedValue({ count: 0 });
    (prisma.contract.count as jest.Mock).mockResolvedValue(0);
    (prisma.note.count as jest.Mock).mockResolvedValue(0);
    (prisma.user.findUnique as jest.Mock).mockResolvedValue({
      first_name: "Max",
      last_name: "Admin",
    });
    (prisma.contract.findFirst as jest.Mock).mockResolvedValue({
      originating_application_id: applicationId,
      applications: [{ id: applicationId }],
    });
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([]);
    (prisma.note.findMany as jest.Mock).mockResolvedValue([]);
    (prisma.application.findUnique as jest.Mock).mockResolvedValue(null);
  });

  it("creates a new Paymaster as UNVERIFIED and writes PAYMASTER_CREATED once", async () => {
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(null);
    (prisma.paymaster.create as jest.Mock).mockResolvedValue(
      paymasterRow({
        id: "pm_new",
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
        registration_number: "202201234567",
        legal_name: "New Co Sdn Bhd",
      })
    );

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId,
      actorUserId,
      applicationId,
      contractId: "ctr-1",
      customerDetails: {
        name: "New Co Sdn Bhd",
        entity_type: "Private Limited Company (Sdn Bhd)",
        ssm_number: "202201234567",
        country: "MY",
        is_related_party: false,
      },
    });

    expect(prisma.paymaster.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          verification_status: "UNVERIFIED",
          registration_number: "202201234567",
        }),
      })
    );
    expect(result.paymasterCreated).toBe(true);
    expect(result.issuerLinkCreated).toBe(false);
    expect(prisma.issuerPaymasterLink.create).toHaveBeenCalled();
    expect(writeLogMock).toHaveBeenCalledTimes(1);
    expect(writeLogMock).toHaveBeenCalledWith(
      expect.objectContaining({
        eventType: ApplicationLogEventType.PAYMASTER_CREATED,
        actorUserId,
        applicationId,
        portal: ActivityPortal.ISSUER,
        paymasterId: "pm_new",
        metadata: expect.objectContaining({
          verification_status: "UNVERIFIED",
          registrationNumber: "202201234567",
        }),
      })
    );
  });

  it("draft persist writes submitted identity without creating a Paymaster", () => {
    const stored = persistDraftCustomerDetails({
      customerDetails: {
        name: "Draft Co Sdn Bhd",
        entity_type: "Private Limited Company (Sdn Bhd)",
        ssm_number: "202201234567",
        country: "MY",
        is_related_party: false,
      },
    });
    expect(stored).toEqual({
      name: "Draft Co Sdn Bhd",
      entity_type: "Private Limited Company (Sdn Bhd)",
      ssm_number: "202201234567",
      country: "MY",
      is_related_party: false,
    });
    expect(stored).not.toHaveProperty("paymaster_id");
    expect(prisma.paymaster.create).not.toHaveBeenCalled();
    expect(prisma.issuerPaymasterLink.create).not.toHaveBeenCalled();
  });

  it("verified amendment persist keeps official identity and related-party from the issuer", () => {
    const stored = persistDraftCustomerDetails({
      previousIdentity: {
        legalName: "Official Co Sdn Bhd",
        entityType: "Private Limited Company (Sdn Bhd)",
        registrationNumber: "202201234567",
        registrationCountry: "MY",
      },
      officialVerifiedIdentity: {
        legalName: "Official Co Sdn Bhd",
        entityType: "Private Limited Company (Sdn Bhd)",
        registrationNumber: "202201234567",
        registrationCountry: "MY",
      },
      customerDetails: {
        name: "Tampered Name",
        entity_type: "Partnership",
        ssm_number: "202201234567",
        country: "SG",
        is_related_party: true,
      },
    });
    expect(stored.name).toBe("Official Co Sdn Bhd");
    expect(stored.entity_type).toBe("Private Limited Company (Sdn Bhd)");
    expect(stored.country).toBe("MY");
    expect(stored.ssm_number).toBe("202201234567");
    expect(stored.is_related_party).toBe(true);
  });

  it("unverified amendment persist writes application identity without a master update", () => {
    const stored = persistDraftCustomerDetails({
      previousIdentity: {
        legalName: "Draft Co Sdn Bhd",
        entityType: "Private Limited Company (Sdn Bhd)",
        registrationNumber: "202201234567",
        registrationCountry: "MY",
      },
      customerDetails: {
        name: "Issuer Typed Name",
        entity_type: "Partnership",
        ssm_number: "202201234567",
        country: "MY",
        is_related_party: false,
      },
    });
    expect(stored.name).toBe("Issuer Typed Name");
    expect(stored.entity_type).toBe("Partnership");
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
  });

  it("frozen persist keeps the previous SSM instead of switching Paymaster", () => {
    const stored = persistDraftCustomerDetails({
      lockSwitching: true,
      retainPaymasterId: "pm_old",
      previousIdentity: {
        legalName: "Old Co",
        entityType: "Private Limited Company (Sdn Bhd)",
        registrationNumber: "111111111111",
        registrationCountry: "MY",
      },
      customerDetails: {
        name: "New Co",
        entity_type: "Partnership",
        ssm_number: "202201234567",
        country: "SG",
        is_related_party: true,
      },
    });
    expect(stored.ssm_number).toBe("111111111111");
    expect(stored.name).toBe("Old Co");
    expect(stored.paymaster_id).toBe("pm_old");
    expect(stored.is_related_party).toBe(true);
    expect(
      resolvePersistedDraftIdentity({
        lockSwitching: false,
        submitted: {
          legalName: "New Co",
          entityType: "Partnership",
          registrationNumber: "202201234567",
          registrationCountry: "SG",
        },
        officialVerifiedIdentity: {
          legalName: "Official New",
          entityType: "Private Limited Company (Sdn Bhd)",
          registrationNumber: "202201234567",
          registrationCountry: "MY",
        },
      }).legalName
    ).toBe("Official New");
  });

  it("draft persist keeps an approved-facility Paymaster id and Admin LPC", () => {
    const stored = persistDraftCustomerDetails({
      previousLargePrivateCompany: true,
      retainPaymasterId: "pm_facility",
      customerDetails: {
        name: "Facility Co",
        entity_type: "Private Limited Company (Sdn Bhd)",
        ssm_number: "202201234567",
        country: "MY",
        is_related_party: true,
      },
    });
    expect(stored.paymaster_id).toBe("pm_facility");
    expect(stored.is_large_private_company).toBe(true);
    expect(shouldRetainLinkedFacilityPaymaster("APPROVED")).toBe(true);
    expect(shouldRetainLinkedFacilityPaymaster("DRAFT")).toBe(false);
    expect(shouldRetainLinkedFacilityPaymaster("AMENDMENT_REQUESTED")).toBe(false);
    expect(shouldLockPaymasterSwitching({ contractStatus: "AMENDMENT_REQUESTED" })).toBe(false);
    expect(
      shouldLockPaymasterSwitching({
        contractStatus: "DRAFT",
        applicationStatus: "INVOICES_SENT",
        invoiceStatuses: ["OFFER_SENT"],
      })
    ).toBe(true);
    expect(
      shouldLockPaymasterSwitching({
        contractStatus: "OFFER_SENT",
        applicationStatus: "CONTRACT_SENT",
      })
    ).toBe(true);
    expect(
      isPaymasterWorkingIdentityEligible({
        applicationStatus: "UNDER_REVIEW",
        financingStructure: { structure_type: "new_contract" },
        contractStatus: "DRAFT",
      })
    ).toBe(true);
    expect(
      isPaymasterWorkingIdentityEligible({
        applicationStatus: "CONTRACT_PENDING",
        financingStructure: { structure_type: "new_contract" },
        contractStatus: "DRAFT",
      })
    ).toBe(true);
    expect(
      isPaymasterWorkingIdentityEligible({
        applicationStatus: "INVOICE_PENDING",
        financingStructure: { structure_type: "invoice_only" },
        contractStatus: "SUBMITTED",
      })
    ).toBe(true);
    expect(
      isPaymasterWorkingIdentityEligible({
        applicationStatus: "UNDER_REVIEW",
        financingStructure: { structure_type: "existing_contract" },
        contractStatus: "APPROVED",
      })
    ).toBe(false);
    expect(
      isPaymasterWorkingIdentityEligible({
        applicationStatus: "CONTRACT_SENT",
        financingStructure: { structure_type: "new_contract" },
        contractStatus: "DRAFT",
      })
    ).toBe(false);
    expect(
      isPaymasterWorkingIdentityEligible({
        applicationStatus: "CONTRACT_PENDING",
        financingStructure: { structure_type: "new_contract" },
        contractStatus: "APPROVED",
      })
    ).toBe(false);
    expect(
      isPaymasterWorkingIdentityEligible({
        applicationStatus: "UNDER_REVIEW",
        financingStructure: { structure_type: "new_contract" },
        contractStatus: "DRAFT",
        hasNote: true,
      })
    ).toBe(false);
  });

  it("reuses a raced unique registration instead of failing create", async () => {
    const existing = paymasterRow({
      id: "pm_raced",
      verification_status: "UNVERIFIED",
      verified_at: null,
      verified_by_user_id: null,
    });
    (prisma.paymaster.findUnique as jest.Mock)
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(existing);
    (prisma.paymaster.create as jest.Mock).mockRejectedValue({ code: "P2002" });
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue({ id: "link-existing" });

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId,
      customerDetails: matchingDetails(existing, { name: "Second Issuer Name" }),
    });

    expect(result.paymasterId).toBe("pm_raced");
    expect(result.paymasterCreated).toBe(false);
    expect(result.customerDetails.name).toBe("Second Issuer Name");
  });

  it("ignores unverified selectedPaymasterId and reuses the SSM master without overwriting submitted identity", async () => {
    const existing = paymasterRow({
      id: "pm_pending",
      verification_status: "UNVERIFIED",
      verified_at: null,
      verified_by_user_id: null,
    });
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(existing);

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId,
      selectedPaymasterId: "pm_pending",
      customerDetails: matchingDetails(existing, { name: "Issuer Typed Name Sdn Bhd" }),
    });

    expect(result.paymasterId).toBe("pm_pending");
    expect(result.customerDetails.name).toBe("Issuer Typed Name Sdn Bhd");
    expect(prisma.paymaster.create).not.toHaveBeenCalled();
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
  });

  it("allows verified selectedPaymasterId without a prior issuer link and emits PAYMASTER_LINKED_TO_ISSUER once", async () => {
    const selected = paymasterRow({ id: "pm_global" });
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(selected);

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId: "org-b",
      selectedPaymasterId: "pm_global",
      actorUserId,
      applicationId,
      contractId: "ctr-2",
      customerDetails: matchingDetails(selected, { is_related_party: true }),
    });

    expect(result.paymasterId).toBe("pm_global");
    expect(result.paymasterCreated).toBe(false);
    expect(result.issuerLinkCreated).toBe(true);
    expect(result.customerDetails.is_related_party).toBe(true);
    expect(prisma.issuerPaymasterLink.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          issuer_organization_id: "org-b",
          paymaster_id: "pm_global",
          is_related_party: true,
        }),
      })
    );
    expect(prisma.paymaster.create).not.toHaveBeenCalled();
    expect(writeLogMock).toHaveBeenCalledTimes(1);
    expect(writeLogMock).toHaveBeenCalledWith(
      expect.objectContaining({
        eventType: ApplicationLogEventType.PAYMASTER_LINKED_TO_ISSUER,
        paymasterId: "pm_global",
      })
    );
  });

  it("does not emit PAYMASTER_LINKED_TO_ISSUER on same-issuer reuse", async () => {
    const selected = paymasterRow();
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(selected);
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue({ id: "link-existing" });

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId,
      selectedPaymasterId: selected.id,
      actorUserId,
      applicationId,
      customerDetails: matchingDetails(selected),
    });

    expect(result.issuerLinkCreated).toBe(false);
    expect(prisma.issuerPaymasterLink.update).toHaveBeenCalled();
    expect(prisma.issuerPaymasterLink.create).not.toHaveBeenCalled();
    expect(writeLogMock).not.toHaveBeenCalled();
  });

  it("does not emit a second link event on repeated cross-issuer Save", async () => {
    const selected = paymasterRow({ id: "pm_global" });
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(selected);
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue({ id: "link-b" });

    await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId: "org-b",
      selectedPaymasterId: "pm_global",
      actorUserId,
      applicationId,
      customerDetails: matchingDetails(selected),
    });
    await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId: "org-b",
      selectedPaymasterId: "pm_global",
      actorUserId,
      applicationId,
      customerDetails: matchingDetails(selected),
    });

    expect(writeLogMock).not.toHaveBeenCalled();
    expect(prisma.issuerPaymasterLink.create).not.toHaveBeenCalled();
  });

  it("auto-links a verified SSM match without requiring selectedPaymasterId", async () => {
    const verified = paymasterRow();
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(verified);
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue({ id: "link-existing" });

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId,
      customerDetails: matchingDetails(verified, { name: "ABC Trading Malaysia" }),
    });

    expect(result.paymasterId).toBe(verified.id);
    expect(result.paymasterCreated).toBe(false);
    expect(result.customerDetails.name).toBe(verified.legal_name);
    expect(prisma.paymaster.create).not.toHaveBeenCalled();
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
  });

  it("reuses an unverified Paymaster without creating a duplicate or mismatch", async () => {
    const existing = paymasterRow({
      id: "pm_unverified",
      verification_status: "UNVERIFIED",
      verified_at: null,
      verified_by_user_id: null,
    });
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(existing);
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue({ id: "link-existing" });

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId,
      actorUserId,
      applicationId,
      customerDetails: matchingDetails(existing),
    });

    expect(prisma.paymaster.create).not.toHaveBeenCalled();
    expect(result.paymasterId).toBe("pm_unverified");
    expect(result.customerDetails.name).toBe(existing.legal_name);
    expect(result.issuerLinkCreated).toBe(false);
    expect(writeLogMock).not.toHaveBeenCalled();
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
  });

  it("reuses an unverified Paymaster and keeps this application's submitted identity", async () => {
    const existing = paymasterRow({
      id: "pm_unverified",
      verification_status: "UNVERIFIED",
      verified_at: null,
      verified_by_user_id: null,
    });
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(existing);
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue({ id: "link-existing" });

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId,
      actorUserId,
      applicationId,
      customerDetails: {
        name: "Different Name Sdn Bhd",
        entity_type: "Public Limited Company (Bhd)",
        ssm_number: "202134567890",
        country: "SG",
        is_related_party: false,
      },
    });

    expect(result.paymasterId).toBe("pm_unverified");
    expect(result.customerDetails.name).toBe("Different Name Sdn Bhd");
    expect(result.customerDetails.entity_type).toBe("Public Limited Company (Bhd)");
    expect(result.customerDetails.country).toBe("SG");
    expect(prisma.paymaster.create).not.toHaveBeenCalled();
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
    expect(writeLogMock).not.toHaveBeenCalled();
  });

  it("links a verified Paymaster and stamps official identity without mutating the master", async () => {
    const selected = paymasterRow();
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(selected);
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue({ id: "link-existing" });

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId,
      selectedPaymasterId: selected.id,
      customerDetails: {
        name: "Wrong Name Sdn Bhd",
        entity_type: "Partnership",
        ssm_number: selected.registration_number,
        country: "SG",
        is_related_party: false,
        is_large_private_company: true,
        document: { s3_key: "cust.pdf" },
      },
    });

    expect(result.paymasterId).toBe(selected.id);
    expect(result.customerDetails.name).toBe(selected.legal_name);
    expect(result.customerDetails.entity_type).toBe(selected.entity_type);
    expect(result.customerDetails.country).toBe(selected.registration_country);
    expect(result.customerDetails.is_related_party).toBe(false);
    expect(result.customerDetails.is_large_private_company).toBe(true);
    expect(result.customerDetails.document).toEqual({ s3_key: "cust.pdf" });
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
    expect(writeLogMock).not.toHaveBeenCalled();
  });

  it("requires an explicit related-party answer", async () => {
    await expect(
      resolvePaymasterFromCustomerDetails({
        issuerOrganizationId,
        customerDetails: {
          name: "ABC Trading Sdn Bhd",
          entity_type: "Private Limited Company (Sdn Bhd)",
          ssm_number: "202134567890",
          country: "MY",
        },
      })
    ).rejects.toMatchObject({
      code: RELATED_PARTY_REQUIRED_CODE,
    } satisfies Partial<AppError>);
  });

  it("keeps related-party issuer-specific on a new link", async () => {
    const selected = paymasterRow({ id: "pm_global" });
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(selected);
    await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId: "org-b",
      selectedPaymasterId: "pm_global",
      customerDetails: matchingDetails(selected, { is_related_party: true }),
    });
    expect(prisma.issuerPaymasterLink.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ is_related_party: true }),
      })
    );
  });

  it("preserves Admin-set large private company on customer_details rebuild", async () => {
    const existing = paymasterRow({ id: "pm_unverified", verification_status: "UNVERIFIED" });
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(existing);
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue({ id: "link-existing" });

    const result = await resolvePaymasterFromCustomerDetails({
      issuerOrganizationId,
      previousLargePrivateCompany: true,
      customerDetails: matchingDetails(existing),
    });

    expect(result.customerDetails.is_large_private_company).toBe(true);
    expect(result.customerDetails.name).toBe(existing.legal_name);
  });

  it("lookup returns master identity only", async () => {
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(paymasterRow());
    const result = await lookupPaymasterByRegistration("202134567890");
    expect(result.status).toBe("FOUND_VERIFIED");
    expect(result.paymaster).toEqual({
      id: "pm_verified",
      legalName: "ABC Trading Sdn Bhd",
      registrationNumber: "202134567890",
      registrationCountry: "MY",
      entityType: "Private Limited Company (Sdn Bhd)",
      verificationStatus: "VERIFIED",
    });
    expect(JSON.stringify(result)).not.toMatch(/issuer/i);
  });

  it("lookup reports unverified and not found without creating rows", async () => {
    (prisma.paymaster.findUnique as jest.Mock)
      .mockResolvedValueOnce(paymasterRow({ verification_status: "UNVERIFIED" }))
      .mockResolvedValueOnce(null);
    await expect(lookupPaymasterByRegistration("202134567890")).resolves.toMatchObject({
      status: "FOUND_UNVERIFIED",
    });
    await expect(lookupPaymasterByRegistration("202201234567")).resolves.toEqual({
      status: "NOT_FOUND",
      paymaster: null,
    });
    expect(prisma.paymaster.create).not.toHaveBeenCalled();
  });

  it("issuer lookup returns unverified masters without exposing identity fields", async () => {
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(
      paymasterRow({
        id: "pm_pending",
        legal_name: "Other Issuer Name",
        verification_status: "UNVERIFIED",
      })
    );
    await expect(lookupIssuerPaymasterByRegistration("202134567890")).resolves.toEqual({
      status: "FOUND_UNVERIFIED",
      paymaster: {
        id: "pm_pending",
        legalName: "",
        registrationNumber: "202134567890",
        registrationCountry: "",
        entityType: "",
        verificationStatus: "UNVERIFIED",
      },
    });
  });

  it("verify automatically overlays official identity onto eligible working applications", async () => {
    const state = {
      row: paymasterRow({
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    const submitted = {
      name: "Typed Name Sdn Bhd",
      entity_type: state.row.entity_type,
      ssm_number: state.row.registration_number,
      country: "MY",
      is_related_party: true,
      is_large_private_company: false,
      document: { s3_key: "cust.pdf" },
      paymaster_id: state.row.id,
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({ details: submitted }),
    ]);
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-1" });

    await verifyPaymaster({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: "ABC Trading Sdn. Bhd.",
        country: "MY",
        entityType: state.row.entity_type,
      },
    });

    expect(prisma.contract.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "ctr-1" },
        data: expect.objectContaining({
          customer_details: expect.objectContaining({
            name: "ABC Trading Sdn. Bhd.",
            is_related_party: true,
            is_large_private_company: false,
            document: { s3_key: "cust.pdf" },
          }),
        }),
      })
    );
    expect(writeLogMock.mock.calls.map((call) => call[0].eventType)).toEqual([
      ApplicationLogEventType.PAYMASTER_IDENTITY_UPDATED,
      ApplicationLogEventType.PAYMASTER_VERIFIED,
      ApplicationLogEventType.PAYMASTER_IDENTITY_SYNCED,
    ]);
    expect(writeLogMock.mock.calls[2]?.[0]).toEqual(
      expect.objectContaining({
        eventType: ApplicationLogEventType.PAYMASTER_IDENTITY_SYNCED,
        actorUserId,
        applicationId,
        portal: ActivityPortal.ADMIN,
        metadata: expect.objectContaining({
          paymaster_id: state.row.id,
          application_id: applicationId,
          contract_id: "ctr-1",
          source: "paymaster_auto_sync",
          trigger: "verification",
          previous: expect.objectContaining({ name: "Typed Name Sdn Bhd" }),
          new: expect.objectContaining({ name: "ABC Trading Sdn. Bhd." }),
        }),
      })
    );
    expect(writeLogMock.mock.calls[2]?.[1]).toBe(prisma);
    expect(prisma.applicationRevision.update).not.toHaveBeenCalled();
  });

  it("verify updates multiple eligible applications and skips frozen ones", async () => {
    const state = {
      row: paymasterRow({
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({
        id: "ctr-review",
        details: matchingDetails(state.row, { name: "ABC Trading Sdn Bhd" }),
        applications: [workingApp("UNDER_REVIEW", { structure_type: "new_contract" }, "app-b")],
      }),
      linkedContract({
        id: "ctr-submitted",
        details: matchingDetails(state.row, { name: "ABC Trading" }),
        applications: [workingApp("SUBMITTED", { structure_type: "invoice_only" }, "app-c")],
      }),
      linkedContract({
        id: "ctr-frozen",
        status: "APPROVED",
        details: matchingDetails(state.row, { name: "ABC TRADING SDN BHD" }),
        applications: [workingApp("COMPLETED", { structure_type: "new_contract" }, "app-a")],
      }),
      linkedContract({
        id: "ctr-facility",
        details: matchingDetails(state.row, { name: "Facility Draw Name" }),
        applications: [workingApp("UNDER_REVIEW", { structure_type: "existing_contract" }, "app-d")],
      }),
    ]);
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-review" });

    await verifyPaymaster({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: "ABC Trading Sdn. Bhd.",
        country: "MY",
        entityType: state.row.entity_type,
      },
    });

    const updatedIds = (prisma.contract.update as jest.Mock).mock.calls.map(
      (call) => call[0].where.id
    );
    expect(updatedIds).toEqual(["ctr-review", "ctr-submitted"]);
    const synced = syncedApplicationIds();
    expect(synced).toEqual(["app-b", "app-c"]);
    expect(writeLogMock.mock.calls.map((call) => call[0].eventType)).toEqual([
      ApplicationLogEventType.PAYMASTER_IDENTITY_UPDATED,
      ApplicationLogEventType.PAYMASTER_VERIFIED,
      ApplicationLogEventType.PAYMASTER_IDENTITY_SYNCED,
      ApplicationLogEventType.PAYMASTER_IDENTITY_SYNCED,
    ]);
    expect(prisma.applicationRevision.update).not.toHaveBeenCalled();
  });

  it("does not rewrite applications that already have a Note", async () => {
    const state = {
      row: paymasterRow({
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({
        id: "ctr-noted",
        details: matchingDetails(state.row, { name: "Old Typed Name" }),
      }),
    ]);
    (prisma.note.findMany as jest.Mock).mockResolvedValue([
      { source_contract_id: "ctr-noted", source_application_id: applicationId },
    ]);

    await verifyPaymaster({ paymasterId: state.row.id, actorUserId });
    expect(prisma.contract.update).not.toHaveBeenCalled();
    expect(syncedApplicationIds()).toEqual([]);
  });

  it("syncs a contract when every linked application is eligible", async () => {
    const state = {
      row: paymasterRow({
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({
        id: "ctr-shared-open",
        details: matchingDetails(state.row, { name: "Typed Shared Name" }),
        applications: [
          workingApp("UNDER_REVIEW", { structure_type: "new_contract" }, "app-open-a"),
          workingApp("SUBMITTED", { structure_type: "invoice_only" }, "app-open-b"),
        ],
      }),
    ]);
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-shared-open" });

    await verifyPaymaster({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: "ABC Trading Sdn. Bhd.",
        country: "MY",
        entityType: state.row.entity_type,
      },
    });

    expect(prisma.contract.update).toHaveBeenCalledTimes(1);
    expect(prisma.contract.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "ctr-shared-open" },
        data: expect.objectContaining({
          customer_details: expect.objectContaining({ name: "ABC Trading Sdn. Bhd." }),
        }),
      })
    );
    expect(syncedApplicationIds()).toEqual(["app-open-a", "app-open-b"]);
  });

  it.each([
    ["CONTRACT_SENT", workingApp("CONTRACT_SENT", { structure_type: "new_contract" }, "app-frozen")],
    ["INVOICES_SENT", workingApp("INVOICES_SENT", { structure_type: "invoice_only" }, "app-frozen")],
    ["COMPLETED", workingApp("COMPLETED", { structure_type: "new_contract" }, "app-frozen")],
    [
      "existing_contract",
      workingApp("UNDER_REVIEW", { structure_type: "existing_contract" }, "app-frozen"),
    ],
  ])(
    "skips the whole contract when an eligible sibling shares it with an ineligible %s application",
    async (_label, ineligibleApp) => {
      const state = {
        row: paymasterRow({
          verification_status: "UNVERIFIED",
          verified_at: null,
          verified_by_user_id: null,
        }),
      };
      mockPaymasterReads(state);
      (prisma.contract.findMany as jest.Mock).mockResolvedValue([
        linkedContract({
          id: "ctr-mixed",
          details: matchingDetails(state.row, { name: "Would Sync If Some" }),
          applications: [
            workingApp("UNDER_REVIEW", { structure_type: "new_contract" }, "app-open"),
            ineligibleApp,
          ],
        }),
      ]);

      await verifyPaymaster({
        paymasterId: state.row.id,
        actorUserId,
        identity: {
          legalName: "ABC Trading Sdn. Bhd.",
          country: "MY",
          entityType: state.row.entity_type,
        },
      });

      expect(prisma.contract.update).not.toHaveBeenCalled();
      expect(syncedApplicationIds()).toEqual([]);
    }
  );

  it("skips the whole contract when one linked application already has a Note", async () => {
    const state = {
      row: paymasterRow({
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({
        id: "ctr-sibling-note",
        details: matchingDetails(state.row, { name: "Would Sync If Some" }),
        applications: [
          workingApp("UNDER_REVIEW", { structure_type: "new_contract" }, "app-open"),
          workingApp("SUBMITTED", { structure_type: "invoice_only" }, "app-noted"),
        ],
      }),
    ]);
    (prisma.note.findMany as jest.Mock).mockResolvedValue([
      { source_contract_id: null, source_application_id: "app-noted" },
    ]);

    await verifyPaymaster({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: "ABC Trading Sdn. Bhd.",
        country: "MY",
        entityType: state.row.entity_type,
      },
    });

    expect(prisma.contract.update).not.toHaveBeenCalled();
    expect(syncedApplicationIds()).toEqual([]);
  });

  it("skips a contract with zero linked applications", async () => {
    const state = {
      row: paymasterRow({
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({
        id: "ctr-orphan",
        details: matchingDetails(state.row, { name: "Orphan Typed Name" }),
        applications: [],
      }),
    ]);

    await verifyPaymaster({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: "ABC Trading Sdn. Bhd.",
        country: "MY",
        entityType: state.row.entity_type,
      },
    });

    expect(prisma.contract.update).not.toHaveBeenCalled();
    expect(syncedApplicationIds()).toEqual([]);
  });

  it("does not sync an approved shared facility with an existing_contract drawdown", async () => {
    const state = {
      row: paymasterRow({
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({
        id: "ctr-approved-shared",
        status: "APPROVED",
        details: matchingDetails(state.row, { name: "ABC TRADING SDN BHD" }),
        applications: [
          workingApp("COMPLETED", { structure_type: "new_contract" }, "app-origin"),
          workingApp("UNDER_REVIEW", { structure_type: "existing_contract" }, "app-draw"),
        ],
      }),
    ]);

    await verifyPaymaster({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: "ABC Trading Sdn. Bhd.",
        country: "MY",
        entityType: state.row.entity_type,
      },
    });

    expect(prisma.contract.update).not.toHaveBeenCalled();
    expect(syncedApplicationIds()).toEqual([]);
  });

  it("does not write PAYMASTER_IDENTITY_SYNCED when working identity already matches", async () => {
    const state = {
      row: paymasterRow({
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({ details: matchingDetails(state.row) }),
    ]);

    await verifyPaymaster({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: state.row.legal_name,
        country: state.row.registration_country,
        entityType: state.row.entity_type,
      },
    });

    expect(prisma.contract.update).not.toHaveBeenCalled();
    expect(writeLogMock.mock.calls.map((call) => call[0].eventType)).toEqual([
      ApplicationLogEventType.PAYMASTER_VERIFIED,
    ]);
  });

  it("blocks sending an offer until Paymaster identity is verified and matches", async () => {
    (prisma.application.findUnique as jest.Mock).mockResolvedValue({
      contract: {
        customer_details: matchingDetails(paymasterRow({ verification_status: "UNVERIFIED" })),
        paymaster: paymasterRow({ verification_status: "UNVERIFIED" }),
      },
    });
    await expect(assertApplicationPaymasterReadyForOffer(applicationId)).rejects.toMatchObject({
      statusCode: 400,
      code: PAYMASTER_NOT_VERIFIED_CODE,
    });
  });

  it("allows an offer once working identity matches the official verified Paymaster", async () => {
    const verified = paymasterRow();
    (prisma.application.findUnique as jest.Mock).mockResolvedValue({
      contract: {
        customer_details: matchingDetails(verified),
        paymaster: verified,
      },
    });
    await expect(assertApplicationPaymasterReadyForOffer(applicationId)).resolves.toBeUndefined();
  });

  it("lists only verified issuer-linked Paymasters", async () => {
    (prisma.issuerPaymasterLink.findMany as jest.Mock).mockResolvedValue([
      {
        is_related_party: false,
        last_used_at: new Date("2026-07-01T00:00:00.000Z"),
        paymaster: paymasterRow({ id: "pm_linked" }),
      },
    ]);
    const rows = await listIssuerPaymasters(issuerOrganizationId);
    expect(prisma.issuerPaymasterLink.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          issuer_organization_id: issuerOrganizationId,
          paymaster: { verification_status: "VERIFIED" },
        },
      })
    );
    expect(rows).toHaveLength(1);
    expect(rows[0]?.id).toBe("pm_linked");
  });

  it("verifyPaymaster from Application Review uses Admin-confirmed identity, not submitted application values", async () => {
    const state = {
      row: paymasterRow({
        id: "pm_shared",
        legal_name: "yayay",
        entity_type: "State Government",
        registration_number: "999999999999",
        registration_country: "MY",
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    const applicationBDetails = {
      name: "bbbb",
      entity_type: "Federal Government Agency",
      ssm_number: "999999999999",
      country: "MY",
      is_related_party: false,
      paymaster_id: "pm_shared",
    };
    mockPaymasterReads(state);
    (prisma.application.findUnique as jest.Mock).mockResolvedValue(
      applicationWithCustomerDetails(applicationBDetails, "pm_shared", "app-b")
    );

    const detail = await verifyPaymaster({
      paymasterId: "pm_shared",
      actorUserId,
      applicationId: "app-b",
      identity: {
        legalName: "Official Trading Sdn. Bhd.",
        country: "MY",
        entityType: "Private Limited Company (Sdn Bhd)",
      },
    });

    expect(prisma.paymaster.update).toHaveBeenCalledWith({
      where: { id: "pm_shared" },
      data: {
        legal_name: "Official Trading Sdn. Bhd.",
        entity_type: "Private Limited Company (Sdn Bhd)",
        registration_country: "MY",
      },
    });
    expect(prisma.paymaster.update).toHaveBeenCalledWith({
      where: { id: "pm_shared" },
      data: {
        verification_status: "VERIFIED",
        verified_at: expect.any(Date),
        verified_by_user_id: actorUserId,
      },
    });
    expect(detail.legalName).toBe("Official Trading Sdn. Bhd.");
    expect(detail.entityType).toBe("Private Limited Company (Sdn Bhd)");
    expect(detail.registrationNumber).toBe("999999999999");
    expect(detail.verificationStatus).toBe("VERIFIED");
    expect(writeLogMock).toHaveBeenCalledTimes(2);
    expect(writeLogMock.mock.calls[0]?.[0]).toEqual(
      expect.objectContaining({
        eventType: ApplicationLogEventType.PAYMASTER_IDENTITY_UPDATED,
        applicationId: "app-b",
        metadata: expect.objectContaining({
          previous: expect.objectContaining({ legalName: "yayay" }),
          new: expect.objectContaining({ legalName: "Official Trading Sdn. Bhd." }),
        }),
      })
    );
    expect(writeLogMock.mock.calls[1]?.[0]).toEqual(
      expect.objectContaining({
        eventType: ApplicationLogEventType.PAYMASTER_VERIFIED,
        applicationId: "app-b",
        metadata: expect.objectContaining({
          source: "application_review",
          legalName: "Official Trading Sdn. Bhd.",
        }),
      })
    );
    expect(prisma.contract.update).not.toHaveBeenCalled();
    expect(prisma.application.update).not.toHaveBeenCalled();
    expect(prisma.applicationRevision.update).not.toHaveBeenCalled();
    expect(prisma.note.update).not.toHaveBeenCalled();
    expect(
      submittedIdentityDiffersFromVerified({
        submitted: applicationBDetails,
        paymaster: state.row,
      })
    ).toBe(true);
  });

  it("rejects verification when the application is not linked to this Paymaster", async () => {
    const state = {
      row: paymasterRow({
        id: "pm_shared",
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.application.findUnique as jest.Mock).mockResolvedValue(
      applicationWithCustomerDetails(
        {
          name: "bbbb",
          entity_type: "Federal Government Agency",
          ssm_number: "111111111111",
          country: "MY",
        },
        "pm_other",
        "app-b"
      )
    );

    await expect(
      verifyPaymaster({ paymasterId: "pm_shared", actorUserId, applicationId: "app-b" })
    ).rejects.toMatchObject({
      statusCode: 400,
      code: PAYMASTER_NOT_LINKED_CODE,
    });
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
    expect(writeLogMock).not.toHaveBeenCalled();
  });

  it("does not overwrite a VERIFIED master from a later conflicting application", async () => {
    const state = {
      row: paymasterRow({
        id: "pm_shared",
        legal_name: "bbbb",
        entity_type: "Federal Government Agency",
        registration_number: "999999999999",
        registration_country: "MY",
        verification_status: "VERIFIED",
      }),
    };
    mockPaymasterReads(state);
    (prisma.application.findUnique as jest.Mock).mockResolvedValue(
      applicationWithCustomerDetails(
        {
          name: "cccc",
          entity_type: "Local Authority",
          ssm_number: "999999999999",
          country: "MY",
        },
        "pm_shared",
        "app-c"
      )
    );

    const detail = await verifyPaymaster({
      paymasterId: "pm_shared",
      actorUserId,
      applicationId: "app-c",
    });
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
    expect(writeLogMock).not.toHaveBeenCalled();
    expect(detail.legalName).toBe("bbbb");
    expect(detail.entityType).toBe("Federal Government Agency");
    expect(detail.verificationStatus).toBe("VERIFIED");
  });

  it("allows Paymaster Detail verify when submitted identities for the SSM differ", async () => {
    const state = {
      row: paymasterRow({
        id: "pm_shared",
        legal_name: "yayay",
        entity_type: "State Government",
        registration_number: "999999999999",
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);

    const detail = await verifyPaymaster({
      paymasterId: "pm_shared",
      actorUserId,
      identity: {
        legalName: "Confirmed Name Sdn Bhd",
        country: "MY",
        entityType: "Private Limited Company (Sdn Bhd)",
      },
    });
    expect(detail.legalName).toBe("Confirmed Name Sdn Bhd");
    expect(detail.verificationStatus).toBe("VERIFIED");
    expect(writeLogMock.mock.calls.map((call) => call[0].eventType)).toEqual([
      ApplicationLogEventType.PAYMASTER_IDENTITY_UPDATED,
      ApplicationLogEventType.PAYMASTER_VERIFIED,
    ]);
    expect(writeLogMock.mock.calls[0]?.[0].applicationId).toBeNull();
  });

  it("verifyPaymaster from Paymaster Detail verifies the current master when submitted identities agree", async () => {
    const state = {
      row: paymasterRow({
        id: "pm_pending",
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      { customer_details: matchingDetails(state.row) },
    ]);

    const detail = await verifyPaymaster({
      paymasterId: "pm_pending",
      actorUserId,
    });
    expect(prisma.paymaster.update).toHaveBeenCalledWith({
      where: { id: "pm_pending" },
      data: {
        verification_status: "VERIFIED",
        verified_at: expect.any(Date),
        verified_by_user_id: actorUserId,
      },
    });
    expect(detail.verificationStatus).toBe("VERIFIED");
    expect(writeLogMock).toHaveBeenCalledTimes(1);
    expect(writeLogMock.mock.calls[0]?.[0]).toEqual(
      expect.objectContaining({
        eventType: ApplicationLogEventType.PAYMASTER_VERIFIED,
        applicationId: null,
        metadata: expect.objectContaining({
          source: "paymaster_detail",
          legalName: "ABC Trading Sdn Bhd",
        }),
      })
    );
  });

  it("verifyPaymaster sets verified_at and writes PAYMASTER_VERIFIED once without approving an application", async () => {
    const state = {
      row: paymasterRow({
        id: "pm_pending",
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.application.findUnique as jest.Mock).mockResolvedValue(
      applicationWithCustomerDetails(matchingDetails(state.row), "pm_pending")
    );

    const detail = await verifyPaymaster({
      paymasterId: "pm_pending",
      actorUserId,
      applicationId,
    });
    expect(prisma.paymaster.update).toHaveBeenCalledWith({
      where: { id: "pm_pending" },
      data: {
        verification_status: "VERIFIED",
        verified_at: expect.any(Date),
        verified_by_user_id: actorUserId,
      },
    });
    expect(detail.verificationStatus).toBe("VERIFIED");
    expect(writeLogMock).toHaveBeenCalledTimes(1);
    expect(writeLogMock.mock.calls[0]?.[0]).toEqual(
      expect.objectContaining({
        eventType: ApplicationLogEventType.PAYMASTER_VERIFIED,
        portal: ActivityPortal.ADMIN,
        metadata: expect.objectContaining({
          previous_status: "UNVERIFIED",
          new_status: "VERIFIED",
          source: "application_review",
        }),
      })
    );
    expect(prisma.application.update).not.toHaveBeenCalled();
    expect(prisma.note.update).not.toHaveBeenCalled();
    expect(prisma.applicationRevision.update).not.toHaveBeenCalled();
  });

  it("does not emit PAYMASTER_VERIFIED on an idempotent second verify", async () => {
    const state = { row: paymasterRow() };
    mockPaymasterReads(state);
    (prisma.application.findUnique as jest.Mock).mockResolvedValue(
      applicationWithCustomerDetails(matchingDetails(state.row), state.row.id)
    );

    await verifyPaymaster({ paymasterId: state.row.id, actorUserId, applicationId });
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
    expect(writeLogMock).not.toHaveBeenCalled();
  });

  it("lets Admin edit an UNVERIFIED master without verifying it", async () => {
    const state = {
      row: paymasterRow({
        id: "pm_pending",
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);

    const detail = await updatePaymasterIdentity({
      paymasterId: "pm_pending",
      actorUserId,
      identity: {
        legalName: "Corrected Name Sdn Bhd",
        country: "SG",
        entityType: "Partnership",
      },
    });

    expect(detail.legalName).toBe("Corrected Name Sdn Bhd");
    expect(detail.registrationCountry).toBe("SG");
    expect(detail.entityType).toBe("Partnership");
    expect(detail.registrationNumber).toBe("202134567890");
    expect(detail.verificationStatus).toBe("UNVERIFIED");
    expect(detail.verifiedAt).toBeNull();
    expect(writeLogMock).toHaveBeenCalledTimes(1);
    expect(writeLogMock.mock.calls[0]?.[0]).toEqual(
      expect.objectContaining({
        eventType: ApplicationLogEventType.PAYMASTER_IDENTITY_UPDATED,
        applicationId: null,
        metadata: expect.objectContaining({
          previous: expect.objectContaining({ legalName: "ABC Trading Sdn Bhd", country: "MY" }),
          new: expect.objectContaining({ legalName: "Corrected Name Sdn Bhd", country: "SG" }),
          verification_status: "UNVERIFIED",
        }),
      })
    );
  });

  it("lets Admin edit a VERIFIED master without changing SSM or verification", async () => {
    const verifiedAt = new Date("2026-06-01T00:00:00.000Z");
    const state = {
      row: paymasterRow({
        verified_at: verifiedAt,
        verified_by_user_id: actorUserId,
      }),
    };
    mockPaymasterReads(state);

    const detail = await updatePaymasterIdentity({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: "ABC Trading Sdn. Bhd.",
        country: "MY",
        entityType: "Private Limited Company (Sdn Bhd)",
      },
    });

    expect(detail.legalName).toBe("ABC Trading Sdn. Bhd.");
    expect(detail.verificationStatus).toBe("VERIFIED");
    expect(detail.verifiedAt).toBe(verifiedAt.toISOString());
    expect(detail.verifiedByUserId).toBe(actorUserId);
    expect(detail.registrationNumber).toBe("202134567890");
    expect(prisma.paymaster.update).toHaveBeenCalledWith({
      where: { id: state.row.id },
      data: {
        legal_name: "ABC Trading Sdn. Bhd.",
        entity_type: "Private Limited Company (Sdn Bhd)",
        registration_country: "MY",
      },
    });
    expect(writeLogMock).toHaveBeenCalledTimes(1);
    expect(writeLogMock.mock.calls[0]?.[0].eventType).toBe(
      ApplicationLogEventType.PAYMASTER_IDENTITY_UPDATED
    );
  });

  it("does not write PAYMASTER_IDENTITY_UPDATED when Admin saves unchanged identity", async () => {
    const state = { row: paymasterRow({ verification_status: "UNVERIFIED" }) };
    mockPaymasterReads(state);
    await updatePaymasterIdentity({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: state.row.legal_name,
        country: state.row.registration_country,
        entityType: state.row.entity_type,
      },
    });
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
    expect(writeLogMock).not.toHaveBeenCalled();
  });

  it("verified-master edit overlays official identity onto eligible applications only", async () => {
    const verifiedAt = new Date("2026-06-01T00:00:00.000Z");
    const state = {
      row: paymasterRow({
        verified_at: verifiedAt,
        verified_by_user_id: actorUserId,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({
        id: "ctr-open",
        details: matchingDetails(state.row),
        applications: [workingApp("AMENDMENT_REQUESTED")],
      }),
      linkedContract({
        id: "ctr-done",
        status: "APPROVED",
        details: matchingDetails(state.row, { name: "ABC TRADING SDN BHD" }),
        applications: [workingApp("COMPLETED")],
      }),
    ]);
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-open" });

    await updatePaymasterIdentity({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: "ABC Trading Sdn. Bhd.",
        country: "MY",
        entityType: state.row.entity_type,
      },
    });

    expect(prisma.contract.update).toHaveBeenCalledTimes(1);
    expect(prisma.contract.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: "ctr-open" },
        data: expect.objectContaining({
          customer_details: expect.objectContaining({ name: "ABC Trading Sdn. Bhd." }),
        }),
      })
    );
    expect(writeLogMock.mock.calls.map((call) => call[0].eventType)).toEqual([
      ApplicationLogEventType.PAYMASTER_IDENTITY_UPDATED,
      ApplicationLogEventType.PAYMASTER_IDENTITY_SYNCED,
    ]);
    expect(writeLogMock.mock.calls[1]?.[0]).toEqual(
      expect.objectContaining({
        eventType: ApplicationLogEventType.PAYMASTER_IDENTITY_SYNCED,
        actorUserId,
        applicationId,
        metadata: expect.objectContaining({
          source: "paymaster_auto_sync",
          trigger: "verified_master_edit",
          previous: expect.objectContaining({ name: "ABC Trading Sdn Bhd" }),
          new: expect.objectContaining({ name: "ABC Trading Sdn. Bhd." }),
        }),
      })
    );
    expect(prisma.applicationRevision.update).not.toHaveBeenCalled();
  });

  it("does not sync working applications when an UNVERIFIED master is edited", async () => {
    const state = {
      row: paymasterRow({
        verification_status: "UNVERIFIED",
        verified_at: null,
        verified_by_user_id: null,
      }),
    };
    mockPaymasterReads(state);
    (prisma.contract.findMany as jest.Mock).mockResolvedValue([
      linkedContract({ details: matchingDetails(state.row, { name: "Issuer Typed" }) }),
    ]);

    await updatePaymasterIdentity({
      paymasterId: state.row.id,
      actorUserId,
      identity: {
        legalName: "Corrected Name Sdn Bhd",
        country: "SG",
        entityType: "Partnership",
      },
    });

    expect(prisma.contract.update).not.toHaveBeenCalled();
    expect(
      writeLogMock.mock.calls.some(
        (call) => call[0].eventType === ApplicationLogEventType.PAYMASTER_IDENTITY_SYNCED
      )
    ).toBe(false);
  });

  it("same-SSM amendment of a verified master stamps official identity", async () => {
    const verified = paymasterRow();
    (prisma.contract.findUnique as jest.Mock).mockResolvedValue({
      id: "ctr-1",
      status: "AMENDMENT_REQUESTED",
      paymaster_id: verified.id,
      customer_details: matchingDetails(verified, {
        is_related_party: true,
        name: "ABC TRADING SDN BHD",
      }),
    });
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(verified);
    (prisma.issuerPaymasterLink.findUnique as jest.Mock).mockResolvedValue({ id: "link-existing" });
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-1" });

    await linkPaymasterForApplicationSubmission({
      contractId: "ctr-1",
      issuerOrganizationId,
      applicationId,
      actorUserId,
    });

    expect(prisma.contract.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          customer_details: expect.objectContaining({
            name: verified.legal_name,
            is_related_party: true,
          }),
          paymaster: { connect: { id: verified.id } },
        }),
      })
    );
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
  });

  it("amendment to a verified new SSM uses that master's official identity", async () => {
    const previous = paymasterRow({ id: "pm_old", registration_number: "111111111111" });
    const next = paymasterRow({
      id: "pm_new",
      legal_name: "Official New Sdn. Bhd.",
      registration_number: "202201234567",
    });
    (prisma.contract.findUnique as jest.Mock).mockResolvedValue({
      id: "ctr-1",
      status: "AMENDMENT_REQUESTED",
      paymaster_id: previous.id,
      customer_details: matchingDetails(next, {
        is_related_party: true,
        name: "Issuer Typed New Name",
      }),
    });
    (prisma.paymaster.findUnique as jest.Mock).mockImplementation(
      async (args: { where?: { id?: string; registration_number?: string } }) => {
        if (args?.where?.id === previous.id) return previous;
        if (args?.where?.registration_number === next.registration_number) return next;
        return null;
      }
    );
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-1" });
    (prisma.contract.count as jest.Mock).mockResolvedValue(0);
    (prisma.note.count as jest.Mock).mockResolvedValue(0);

    await linkPaymasterForApplicationSubmission({
      contractId: "ctr-1",
      issuerOrganizationId,
      applicationId,
      actorUserId,
    });

    expect(prisma.contract.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          customer_details: expect.objectContaining({
            name: "Official New Sdn. Bhd.",
            is_related_party: true,
          }),
          paymaster: { connect: { id: "pm_new" } },
        }),
      })
    );
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
  });

  it("removes a stale issuer link when amendment switches to a different Paymaster", async () => {
    const previous = paymasterRow({ id: "pm_old", registration_number: "111111111111" });
    const next = paymasterRow({
      id: "pm_new",
      registration_number: "202201234567",
      verification_status: "UNVERIFIED",
    });
    (prisma.contract.findUnique as jest.Mock).mockResolvedValue({
      id: "ctr-1",
      status: "AMENDMENT_REQUESTED",
      paymaster_id: previous.id,
      customer_details: matchingDetails(next, { is_related_party: false }),
    });
    (prisma.paymaster.findUnique as jest.Mock).mockImplementation(
      async (args: { where?: { id?: string; registration_number?: string } }) => {
        if (args?.where?.id === previous.id) return previous;
        if (args?.where?.registration_number === next.registration_number) return next;
        return null;
      }
    );
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-1" });
    (prisma.contract.count as jest.Mock).mockResolvedValue(0);
    (prisma.note.count as jest.Mock).mockResolvedValue(0);

    await linkPaymasterForApplicationSubmission({
      contractId: "ctr-1",
      issuerOrganizationId,
      applicationId,
      actorUserId,
    });

    expect(prisma.contract.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          paymaster: { connect: { id: "pm_new" } },
        }),
      })
    );
    expect(prisma.issuerPaymasterLink.deleteMany).toHaveBeenCalledWith({
      where: {
        issuer_organization_id: issuerOrganizationId,
        paymaster_id: "pm_old",
      },
    });
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
  });

  it("keeps the previous issuer link when another facility still uses the old Paymaster", async () => {
    const previous = paymasterRow({ id: "pm_old", registration_number: "111111111111" });
    const next = paymasterRow({ id: "pm_new", registration_number: "202201234567" });
    (prisma.contract.findUnique as jest.Mock).mockResolvedValue({
      id: "ctr-1",
      status: "AMENDMENT_REQUESTED",
      paymaster_id: previous.id,
      customer_details: matchingDetails(next, { is_related_party: false }),
    });
    (prisma.paymaster.findUnique as jest.Mock).mockImplementation(
      async (args: { where?: { id?: string; registration_number?: string } }) => {
        if (args?.where?.id === previous.id) return previous;
        if (args?.where?.registration_number === next.registration_number) return next;
        return null;
      }
    );
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-1" });
    (prisma.contract.count as jest.Mock).mockResolvedValue(1);

    await linkPaymasterForApplicationSubmission({
      contractId: "ctr-1",
      issuerOrganizationId,
      applicationId,
      actorUserId,
    });

    expect(prisma.issuerPaymasterLink.deleteMany).not.toHaveBeenCalled();
  });

  it("does not retarget Paymaster after an invoice offer even if the holder is still draft", async () => {
    const previous = paymasterRow({ id: "pm_old", registration_number: "111111111111" });
    const next = paymasterRow({ id: "pm_new", registration_number: "202201234567" });
    (prisma.contract.findUnique as jest.Mock).mockResolvedValue({
      id: "ctr-1",
      status: "DRAFT",
      paymaster_id: previous.id,
      customer_details: matchingDetails(next, { is_related_party: false }),
    });
    (prisma.application.findUnique as jest.Mock).mockResolvedValue({
      status: "INVOICES_SENT",
      invoices: [{ status: "OFFER_SENT" }],
    });
    (prisma.paymaster.findUnique as jest.Mock).mockImplementation(
      async (args: { where?: { id?: string; registration_number?: string } }) => {
        if (args?.where?.id === previous.id) return previous;
        if (args?.where?.registration_number === next.registration_number) return next;
        return null;
      }
    );
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-1" });

    await linkPaymasterForApplicationSubmission({
      contractId: "ctr-1",
      issuerOrganizationId,
      applicationId,
      actorUserId,
    });

    expect(prisma.contract.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          paymaster: { connect: { id: "pm_old" } },
        }),
      })
    );
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
  });

  it("does not retarget Paymaster when the facility is locked", async () => {
    const previous = paymasterRow({ id: "pm_old", registration_number: "111111111111" });
    const next = paymasterRow({ id: "pm_new", registration_number: "202201234567" });
    (prisma.contract.findUnique as jest.Mock).mockResolvedValue({
      id: "ctr-1",
      status: "SUBMITTED",
      paymaster_id: previous.id,
      customer_details: matchingDetails(next, { is_related_party: false }),
    });
    (prisma.paymaster.findUnique as jest.Mock).mockImplementation(
      async (args: { where?: { id?: string; registration_number?: string } }) => {
        if (args?.where?.id === previous.id) return previous;
        if (args?.where?.registration_number === next.registration_number) return next;
        return null;
      }
    );
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-1" });

    await linkPaymasterForApplicationSubmission({
      contractId: "ctr-1",
      issuerOrganizationId,
      applicationId,
      actorUserId,
    });

    expect(prisma.contract.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          paymaster: { connect: { id: "pm_old" } },
        }),
      })
    );
    expect(prisma.paymaster.update).not.toHaveBeenCalled();
    expect(prisma.issuerPaymasterLink.deleteMany).not.toHaveBeenCalled();
  });

  it("locked facility submit keeps the frozen identity even when the master is verified", async () => {
    const previous = paymasterRow({
      id: "pm_old",
      legal_name: "ABC TRADING SDN BHD",
      registration_number: "111111111111",
    });
    (prisma.contract.findUnique as jest.Mock).mockResolvedValue({
      id: "ctr-1",
      status: "APPROVED",
      paymaster_id: previous.id,
      customer_details: matchingDetails(previous, {
        is_related_party: false,
        name: "ABC TRADING SDN BHD",
      }),
    });
    (prisma.paymaster.findUnique as jest.Mock).mockResolvedValue(previous);
    (prisma.contract.update as jest.Mock).mockResolvedValue({ id: "ctr-1" });

    await linkPaymasterForApplicationSubmission({
      contractId: "ctr-1",
      issuerOrganizationId,
      applicationId,
      actorUserId,
    });

    expect(prisma.contract.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          customer_details: expect.objectContaining({ name: "ABC TRADING SDN BHD" }),
          paymaster: { connect: { id: "pm_old" } },
        }),
      })
    );
  });
});

describe("Paymaster identity writers stay notification-free and separate from approval", () => {
  const { readFileSync } = require("node:fs") as typeof import("node:fs");
  const { join } = require("node:path") as typeof import("node:path");

  it("does not send Paymaster notifications from create, link, or verify", () => {
    const src = [
      readFileSync(join(__dirname, "service.ts"), "utf8"),
      readFileSync(join(__dirname, "identity-audit.ts"), "utf8"),
      readFileSync(join(__dirname, "controller.ts"), "utf8"),
      readFileSync(join(__dirname, "activity.ts"), "utf8"),
      readFileSync(join(__dirname, "../admin/controller.ts"), "utf8"),
    ].join("\n");
    expect(src).not.toMatch(/sendTyped|NotificationService|createInternal/);
    expect(src).toMatch(/adminPaymasterRouter.patch/);
    expect(src).toMatch(/updatePaymasterIdentity/);
    expect(src).toMatch(/syncOfficialIdentityToEligibleApplications/);
    expect(src).toMatch(/isPaymasterWorkingIdentityContractEligible/);
    expect(src).toMatch(/PAYMASTER_IDENTITY_SYNCED/);
    expect(src).not.toMatch(/eventType:\s*ApplicationLogEventType\.PAYMASTER_IDENTITY_RESOLVED/);
    expect(src).not.toMatch(/use-verified|Use Verified Paymaster Details/);
  });

  it("does not copy application submitted identity onto the master during verify", () => {
    const src = readFileSync(join(__dirname, "service.ts"), "utf8");
    const verifyStart = src.indexOf("export async function verifyPaymaster");
    const verifyEnd = src.indexOf("export async function listIssuerPaymasters");
    const verifyFn = src.slice(verifyStart, verifyEnd);
    expect(verifyFn).toMatch(/requireOfficialIdentity/);
    expect(verifyFn).toMatch(/applyOfficialIdentityUpdate/);
    expect(verifyFn).toMatch(/syncOfficialIdentityToEligibleApplications/);
    expect(verifyFn).not.toMatch(/submittedFromApplication/);
    expect(verifyFn).not.toMatch(/PAYMASTER_IDENTITY_IMMUTABLE/);
  });

  it("defers Paymaster create/link from contract draft save to application submit", () => {
    const contractSrc = readFileSync(join(__dirname, "../contracts/service.ts"), "utf8");
    const applicationSrc = readFileSync(join(__dirname, "../applications/service.ts"), "utf8");
    const amendmentSrc = readFileSync(join(__dirname, "../applications/amendments/service.ts"), "utf8");
    expect(contractSrc).toMatch(/persistDraftCustomerDetails/);
    expect(contractSrc).not.toMatch(/resolvePaymasterFromCustomerDetails/);
    expect(applicationSrc).toMatch(/linkPaymasterForApplicationSubmission/);
    expect(amendmentSrc).toMatch(/linkPaymasterForApplicationSubmission/);
    expect(contractSrc).toMatch(/shouldLockPaymasterSwitching/);
    expect(contractSrc).toMatch(/officialVerifiedIdentity/);
  });

  it("does not verify a Paymaster when an application or section is approved", () => {
    const adminSrc = readFileSync(join(__dirname, "../admin/service.ts"), "utf8");
    expect(adminSrc).not.toMatch(/verifyPaymaster/);
    expect(adminSrc).not.toMatch(/verification_status:\s*"VERIFIED"/);
    expect(adminSrc).toMatch(/ensureCustomerPaymasterAmendmentAllowed/);
    expect(adminSrc).toMatch(/isPaymasterSwitchingFrozen/);
    expect(adminSrc).toMatch(/Paymaster cannot be changed after a commercial offer or signed facility/);
  });
});
