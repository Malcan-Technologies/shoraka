...
router.get("/login", async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const session = req.session as typeof req.session & { requestedRole?: string; signup?: boolean };
    session.nonce = nonce;
    session.state = state;
...
router.get("/callback", async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    logger.info(
      { correlationId, hasSession: !!req.session, nonce: !!req.session?.nonce, state: !!req.session?.state },
    ...
    if (req.query.error) {
      ...
      if (isSignup && (errorDescription?.toLowerCase().includes("already exists") || errorDescription?.toLowerCase().includes("user already exists"))) {
        logger.info({ correlationId, email: errorDescription }, "User already exists, redirecting to login");
        const env = getEnv();
        const loginUrl = new URL(`${env.FRONTEND_URL}/get-started`);
        loginUrl.searchParams.set("error", "user_exists");
        loginUrl.searchParams.set("message", "An account with this email already exists. Please sign in instead.");
    ...
    if (req.session?.state) {
      if (stateFromQuery !== req.session.state) {
        logger.error({ correlationId, sessionState: req.session.state, queryState: stateFromQuery }, "State mismatch");
        throw new AppError(400, "INVALID_STATE", "State mismatch - possible CSRF attack");
      }
    } else {
      logger.warn({ correlationId }, "Session state not found, proceeding without state validation");
    }
    ...
    try {
      ...
      logger.error({ correlationId, error: error instanceof Error ? error.message : String(error) }, "Token exchange failed");
      throw new AppError(400, "TOKEN_EXCHANGE_FAILED", `Token exchange failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    try {
      ...
      logger.info({ correlationId, hasSub: !!userInfo.sub, hasEmail: !!userInfo.email }, "User info retrieved");
    } catch (error) {
      logger.error({ correlationId, error: error instanceof Error ? error.message : String(error) }, "Failed to get user info");
      throw new AppError(400, "USER_INFO_FAILED", `Failed to get user info: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    if (requestedRoleStr) {
      ...
      if (upperRole === "INVESTOR") {
        ...
        logger.warn({ correlationId, requestedRoleStr, upperRole }, "Invalid requestedRole in session, defaulting to INVESTOR");
      }
    } else {
      logger.warn({ correlationId, sessionKeys: session ? Object.keys(session) : [] }, "No requestedRole in session, defaulting to INVESTOR");
    }
    ...
    if (!user) {
      ...
      if (user) {
        logger.info({ correlationId, userId: user.id, email }, "User found by email, updating cognito_sub");
    ...
    if (!user) {
      ...
      try {
        ...
        logger.info({ correlationId, userId: user.id, email, roles: initialRoles, requestedRole, isSignup }, "User created in database (roles will be added after onboarding)");
      } catch (error: any) {
        if (error?.code === "P2002") {
          logger.warn({ correlationId, error }, "User creation failed due to unique constraint, attempting to find existing user");
          user = await prisma.user.findUnique({
            where: { email },
          });
    ...
    if (error instanceof z.ZodError) {
      return next(new AppError(400, "VALIDATION_ERROR", "Missing authorization code", error.errors));
    }
...
router.get("/logout", async (req: Request, res: Response) => {
  ...
  const token = tokenFromCookie || (authHeader?.startsWith("Bearer ") ? authHeader.substring(7) : undefined);
  ...
  if (cognitoSub) {
    try {
      ...
      logger.info({ correlationId, cognitoSub }, "User signed out from Cognito successfully via AdminUserGlobalSignOut");
    } catch (error) {
      ...
      logger.warn(
        { correlationId, cognitoSub, error: error instanceof Error ? error.message : String(error) },
  ...
  logger.info({ correlationId, logoutUrl, userId, portal }, "Redirecting to Cognito logout endpoint");
