...
| `contract_offer_sent` | Facility Offer Sent | Issuer | Platform + email | Yes | Automatic | Admin sends facility offer (`AdminService`). | Issuer org owner + Owner/Org Admin members. |
...
| `offer_retracted_or_reset` | Offer Retracted or Reset | Issuer | Platform only (email off in seed) | Yes | Automatic | Admin retracts or resets a sent facility or invoice offer (`AdminService`). | Issuer org owner + Owner/Org Admin members. |
