import { computeMarketplaceCommitBounds } from "./note-money";
import {
...
export function formatMarketplaceCurrency(amount: number): string {
  const rounded = roundNoteMoney(amount);
...
export function marketplaceFundingSummary(note: MarketplaceNote): string {
  ...
  return `${note.fundingPercent}% funded · ${note.minimumFundingPercent}% min · ${formatMarketplaceCurrency(note.remainingCapacity)} still open`;
}
...
export function marketplaceInvestAnyAmountLabel(note: MarketplaceNote): string {
  return `Invest any amount from ${formatMarketplaceCurrency(note.minInvestment)} to ${formatMarketplaceCurrency(note.maxInvestment)}`;
}
...
export function marketplaceInvestorSummary(note: MarketplaceNote): string {
  return formatNoteInvestorCommitment(
    formatMarketplaceCurrency(note.fundedAmount),
    note.investorCount
  );
}
